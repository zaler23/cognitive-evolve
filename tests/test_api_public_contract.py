...
from cognitive_evolve_runtime.api.jobs import JobQueue, _get_job, _job_public, _set_job, _status_from_nexus_data, _task_dir_for_request
...
from cognitive_evolve_runtime.nexus.state import nexus_runtime_state
...
def test_api_preliminary_validation_is_evidence_based_and_integrity_gated() -> None:
    ...
    assert _nexus_verification_passed(_solved_nexus(objective_solved=True, graded_output=_graded_output_portfolio())) is True
    assert _nexus_verification_passed(_solved_nexus(objective_solved=False, graded_output=_graded_output_verified())) is True
    ...
    assert _nexus_verification_passed(_fake_nexus(1)) is None


def test_completion_payload_exposes_preliminary_state_without_claiming_solution() -> None:
    ...
    assert payload["cognitive_evolve"]["preliminary_validation_passed"] is True
    assert payload["cognitive_evolve"]["validation_status"] == "preliminary_passed"
    assert payload["cognitive_evolve"]["objective_solved"] is False
    assert stale_payload["cognitive_evolve"]["preliminary_validation_passed"] is True
    assert stale_payload["cognitive_evolve"]["objective_solved"] is False
    assert "verification_passed" not in payload["cognitive_evolve"]
...
def test_openai_compatible_app_health_models_and_completion(tmp_path: Path) -> None:
    ...
    script = textwrap.dedent(
        ...
        from cognitive_evolve_runtime.api.jobs import _set_job
        ...
        _set_job(
            ...
            prompt="Audit API",
            answer="API answer",
            nexus_data={"mode": "text", "evolution": {"progress_events": []}, "verification_summaries": []},
        )
        with TestClient(openai_compat.create_app()) as client:
            ...
            job_result = client.get("/v1/cogev/jobs/job-result/result")
        ...
        print(json.dumps({
            ...
            "job_result_status": job_result.status_code,
            "job_result_answer": job_result.json()["choices"][0]["message"]["content"],
        }))
        """
    )
    env = os.environ.copy()
    ...
    assert data["job_result_status"] == 200
    assert data["job_result_answer"] == "API answer"
...
def test_jobs_registry_public_rehydration_and_status_cli(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert public["artifact_ref"] == "job-test"
    assert "task_dir" not in public
...
def test_route_incomplete_runtime_state_and_job_status_stay_visible(tmp_path: Path) -> None:
    nexus_data = {
        "mode": "text",
        "evolution": {
            "completion_status": "route_incomplete",
            "progress_events": [{"round": 1}],
            "synthesis": {
                "status": "route_incomplete",
                "completion_status": "route_incomplete",
                "final_answer": "best current direction",
                "answer_produced": True,
                "closure_certificate": {"objective_solved": False, "critical_failures": [], "answer_produced": True},
            },
        },
        "verification_summaries": [],
    }
    state = nexus_runtime_state(task_dir=tmp_path, prompt="p", run_data=nexus_data, status="route_incomplete")
    task_dir = _task_dir_for_request(tmp_path, "job-route-incomplete")
    job = _set_job(
        "job-route-incomplete",
        status=_status_from_nexus_data(nexus_data),
        task_dir=str(task_dir),
        artifact_root=str(task_dir),
        answer="best current direction",
        nexus_data=nexus_data,
    )
    public = _job_public(job)

    assert state["status"] == "route_incomplete"
    assert state["nexus_evolution"]["completion_status"] == "route_incomplete"
    assert public["status"] == "route_incomplete"
    assert public["cognitive_evolve"]["completion_status"] == "route_incomplete"


def test_resume_available_tracks_checkpoint_existence(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("COGEV_API_TASK_ROOT", str(tmp_path))
    with_checkpoint = _task_dir_for_request(tmp_path, "job-with-checkpoint")
    (with_checkpoint / "nexus-runtime").mkdir(parents=True)
    (with_checkpoint / "nexus-runtime" / "checkpoint.json").write_text("{}", encoding="utf-8")
    without_checkpoint = _task_dir_for_request(tmp_path, "job-without-checkpoint")

    available = _job_public(_set_job("job-with-checkpoint", status="route_incomplete", task_dir=str(with_checkpoint), artifact_root=str(with_checkpoint)))
    unavailable = _job_public(_set_job("job-without-checkpoint", status="route_incomplete", task_dir=str(without_checkpoint), artifact_root=str(without_checkpoint)))

    assert available["resume_available"] is True
    assert available["durable_resume_plan"]["status"] == "snapshot_only"
    assert unavailable["resume_available"] is False

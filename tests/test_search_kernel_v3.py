...
from cognitive_evolve_runtime.nexus.loop.seeding import _generate_model_seed_batches, seed_population
...
class _SeedModel:
    ...
    def seed_population(self, *, contract, world, policy):
        self.calls += 1
        self.requested_counts.append(int(getattr(policy, "metadata", {}).get("requested_candidate_count") or 0))
...
def test_seed_generation_requests_target_and_stops_after_first_batch(monkeypatch) -> None:
    monkeypatch.delenv("COGEV_NEXUS_SEED_MIN_BATCHES", raising=False)
    monkeypatch.delenv("COGEV_NEXUS_SEED_BATCH_LIMIT", raising=False)
    model = _SeedModel()
    accepted, rejected, error = _generate_model_seed_batches(
        model=model,
        contract=_Contract(),
        world=_World(),
        policy=EvolutionPolicy(metadata={"initial_candidate_count": 2}),
        target_size=2,
    )
    assert error is None
    assert model.calls == 1
    assert model.requested_counts == [2]
    assert [candidate.metadata["model_claimed_candidate_id"] for candidate in accepted] == ["A"]
...
def test_operator_incumbent_is_generation_zero_and_reduces_model_seed_target(monkeypatch) -> None:
    monkeypatch.delenv("COGEV_NEXUS_SEED_BATCH_LIMIT", raising=False)
    model = _SeedModel()
    contract = NexusObjectiveContract(original_user_goal="improve incumbent", normalized_goal="improve incumbent")
    incumbent = CandidateGenome(id="I", parent_ids=["OLD"], generation=7, lineage=["OLD", "I"], artifact="incumbent", concise_claim="incumbent", core_mechanism="baseline")
    ...
    assert population.candidates[0].id == "I"
    assert population.candidates[1].metadata["model_claimed_candidate_id"] == "A"
    assert model.requested_counts == [1]
    assert incumbent.parent_ids == []
    assert incumbent.generation == 0
    assert incumbent.lineage == ["I"]
    assert incumbent.contract_hash == contract.contract_hash()
    assert incumbent.metadata["operator_provided_incumbent"] is True


def test_empty_model_seed_result_stays_empty_and_records_fatal_error(monkeypatch) -> None:
    monkeypatch.delenv("COGEV_NEXUS_SEED_BATCH_LIMIT", raising=False)
    policy = EvolutionPolicy()
    population = seed_population(
        contract=NexusObjectiveContract(original_user_goal="seed", normalized_goal="seed"),
        world=_World(),
        policy=policy,
        model=_EmptySeedModel(),
        min_population_size=2,
    )

    assert population.candidates == []
    assert policy.metadata["seed_harvest"]["stopped_reason"] == "model_error"
    assert "no valid candidates" in policy.metadata["seed_harvest"]["fatal_model_error"]


def test_empty_model_seed_fill_keeps_operator_incumbent_runnable(monkeypatch) -> None:
    monkeypatch.delenv("COGEV_NEXUS_SEED_BATCH_LIMIT", raising=False)
    policy = EvolutionPolicy()
    incumbent = CandidateGenome(id="I", artifact={"answer": "incumbent"})
    ...
    assert [candidate.id for candidate in population.candidates] == ["I"]
...
def test_seed_generation_records_single_model_origin_without_changing_acceptance(monkeypatch) -> None:
    ...
    monkeypatch.setenv("COGEV_NEXUS_SEED_BATCH_LIMIT", "2")
    ...
    assert error is None
    assert [candidate.metadata["model_claimed_candidate_id"] for candidate in accepted] == ["A", "C"]
    assert rejected
...
def test_offspring_partial_model_success_survives_recoverable_later_error(monkeypatch) -> None:
    ...
    assert any(candidate.metadata.get("model_claimed_candidate_id") == "M1" for candidate in offspring)
    assert all(candidate.id != "M1" for candidate in offspring)
...
def test_plan_and_offspring_default_to_one_model_batch(monkeypatch) -> None:
    monkeypatch.delenv("COGEV_NEXUS_MUTATION_PLAN_BATCH_LIMIT", raising=False)
    monkeypatch.delenv("COGEV_NEXUS_OFFSPRING_BATCH_LIMIT", raising=False)
    assert _mutation_plan_batch_limit(8) == 1
    assert _offspring_batch_limit(8) == 1

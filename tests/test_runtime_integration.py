...
def test_fixture_backed_runtime_smoke_passes_core_and_runtime_doctor(tmp_path, monkeypatch) -> None:
    ...
    assert state["verification_results"]["preliminary_validation_passed"] is None
    assert "passed" not in state["verification_results"]
    assert state["verification_results"]["validation_status"] == "not_run"
    assert state["verification_results"]["runtime_integrity_passed"] is True
    assert state["verification_results"]["objective_solved"] is False

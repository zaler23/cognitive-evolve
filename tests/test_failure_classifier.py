...
def test_recovery_eligibility_keeps_malformed_diff_with_current_target_repairable() -> None:
    candidate = CandidateGenome(
        id="recoverable-diff",
        artifact_type="code_patch",
        artifact={"path": "cognitive_evolve_runtime/nexus/loop/__init__.py"},
        failure_lessons=["patch_application_failed", "unified_patch_failed:patch: **** unexpected end of file in patch"],
        source_bindings=[{"path": "cognitive_evolve_runtime/nexus/loop/__init__.py", "kind": "source_file"}],
    )
    candidate.patch_application_result = {
        "status": "failed",
        "diagnostics": ["unified_patch_failed:patch: **** unexpected end of file in patch"],
        "failed_files": ["cognitive_evolve_runtime/nexus/loop/__init__.py"],
    }

    verdict = classify_recovery_eligibility(candidate)

    assert verdict.repairable is True
    assert verdict.repair_targets == ["cognitive_evolve_runtime/nexus/loop/__init__.py"]
...
def test_classifier_keeps_documentation_only_targets_terminal() -> None:
    docs_candidate = CandidateGenome(id="docs-only", artifact_type="code_patch", artifact={"path": "docs/design-note.md"})
    root_note_candidate = CandidateGenome(id="root-note", artifact_type="code_patch", artifact={"path": "design-note.md"})

    assert classify_candidate_failure(docs_candidate, {"passed": False}).category == "terminal_documentation_only_patch"
    assert classify_candidate_failure(root_note_candidate, {"passed": False}).category == "terminal_documentation_only_patch"

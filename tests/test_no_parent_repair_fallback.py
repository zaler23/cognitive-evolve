    ...
    parent = CandidateGenome(
        ...
        metadata={
            ...
            "stage_eligibility": {
                ...
                "hard_reject_reason": "hard_reject_diagnostic:runtime_code_change_required,runtime_code_change_absent:documentation_only_patch",
            },
        },
...
def test_documentation_only_diagnosis_uses_ranked_repair_parent_when_pool_is_empty() -> None:
    parent = CandidateGenome(
        id="docs-loop-seed",
        current_fate=CandidateFate.DORMANT,
        artifact_type="project_patch",
        concise_claim="documentation-only project candidate needs code repair",
        core_mechanism="documentation_only_patch",
        failure_lessons=["project patch did not touch the required implementation or test surface"],
        metadata={
            "failure_micro_guidance": [
                {
                    "blocker": "runtime_code_change_absent:documentation_only_patch",
                    "next_action": "replace the documentation-only patch with a concrete runtime/test/schema patch",
                    "evidence_needed": ["runtime_or_test_patch", "source_binding"],
                }
            ],
            "stage_eligibility": {"parent_eligible": False, "hard_reject_reason": "runtime_code_change_absent:documentation_only_patch"},
        },
        verification_result={"passed": False, "diagnostics": ["runtime_code_change_absent:documentation_only_patch", "runtime_code_change_required"]},
    )
    population = CandidatePopulation([parent])
    diagnosis = SearchDiagnosis(
        stagnation_detected=True,
        ...
        notes="Exploration is trapped in documentation-only changes rather than implementation patches.",
    )
    round_stage = EvolutionRound(model=None, budget=EvolutionBudget(max_rounds=48, branch_factor=2))

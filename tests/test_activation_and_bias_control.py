...
def test_relative_rater_runs_reversed_ab_pass_when_explicitly_enabled(monkeypatch) -> None:
    monkeypatch.setenv("COGEV_RANK_ORDER_AUDIT", "always")
...
def test_relative_rater_uses_one_model_call_by_default(monkeypatch) -> None:
    monkeypatch.delenv("COGEV_RANK_ORDER_AUDIT", raising=False)
    candidates = [
        CandidateGenome(id="A", current_fate=CandidateFate.ACTIVE.value),
        CandidateGenome(id="B", current_fate=CandidateFate.ACTIVE.value),
    ]
    model = OrderBiasedRankModel()

    RelativeRater(model=model).rank(candidates=candidates, contract=None, policy=None, archives=ArchiveManager())

    assert model.orders == [["A", "B"]]


def test_relative_rater_does_not_enable_audit_for_unknown_mode(monkeypatch) -> None:
    monkeypatch.setenv("COGEV_RANK_ORDER_AUDIT", "typo")
    candidates = [CandidateGenome(id="A"), CandidateGenome(id="B")]
    model = OrderBiasedRankModel()

...
def test_model_adapter_facade_is_not_schema_or_repair_host() -> None:
    ...
    assert "StructuredModelAdapter" in text
    assert "model_adapter_facets" not in text
    ...
    assert not (ROOT / "cognitive_evolve_runtime" / "nexus" / "model_adapter_facets").exists()

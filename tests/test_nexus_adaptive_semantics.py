...
class BatchSeedModel:
    ...
    def generate_offspring(self, *, plans: list[Any], parents: list[CandidateGenome], **_: Any) -> list[dict[str, Any]]:
        ...
        for index, plan in enumerate(plans):
            ...
            out.append(
                {
                    **_candidate(f"{parent.id}-g{parent.generation + 1}-{index}", "model-refinement"),
                    "parent_ids": parent_ids,
                    "metadata": {"plan_id": str((getattr(plan, "metadata", {}) or {}).get("plan_id") or "")},
                }
            )
        return out
...
def test_infer_task_type_uses_prompt_length_without_signals() -> None:
    from cognitive_evolve_runtime.nexus.semantics import infer_task_type

    assert infer_task_type("short") == "direct_answer_or_small_edit"

...
def test_unmeasured_preliminary_pass_does_not_override_higher_quality_candidate() -> None:
    weak_checked = CandidateGenome(
        id="weak-checked",
        current_fate=CandidateFate.ACTIVE.value,
        concise_claim="Weak checked answer",
        verification_result={"passed": True},
        multihead_scores={"answer_likelihood": 0.0, "objective_alignment": 0.0, "core_mechanism_strength": 0.0},
    )
    strong = CandidateGenome(
        id="strong",
        current_fate=CandidateFate.ACTIVE.value,
        concise_claim="Strong direct answer with a concrete mechanism and falsification path.",
        core_mechanism="Concrete mechanism",
        multihead_scores={
    ...
    result = synthesize_result(population=CandidatePopulation([weak_checked, strong]), archives=ArchiveManager())
    projection = build_final_projection(
        population=CandidatePopulation([weak_checked, strong]),
    ...
    assert projection.candidate_id == "strong"
    assert projection.best_current_direction["route"] == "best_current"
...
def test_user_facing_verification_does_not_trust_candidate_metadata_verified() -> None:
    ...
    assert projection.best_current_direction["validation_status"] == "not_run"
...
def test_final_projection_binds_synthesis_answer_to_best_current_candidate_id() -> None:
    ...
    assert projection.artifact == candidate.artifact
    assert projection.evidence_summary["answer_candidate_mismatch"] is True
    assert "projection_answer_candidate_mismatch" in projection.advisory_issues
    assert projection.best_current_direction["candidate_id"] == "chosen"
    assert projection.best_current_direction["external_review_required_reason"] != "answer_unbound_to_candidate_artifact"
    assert "answer_unbound_to_candidate_artifact" not in projection.advisory_issues


def test_final_projection_rejects_unbound_model_answer_as_artifact() -> None:
    candidates = [
    ...
    assert projection.candidate_id in {"candidate-a", "candidate-b"}
    assert projection.artifact != synthesis.final_answer
    assert projection.best_current_direction["candidate_id"] == projection.candidate_id
...
def test_unbound_legacy_graded_result_does_not_bless_candidate() -> None:
...
def test_duplicate_offspring_is_hard_excluded_with_budget_trace() -> None:
    ...
    assert child.metadata["candidate_budget_decision"]["reason"] == "duplicate_materialized_artifact"

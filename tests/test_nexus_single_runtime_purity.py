...
def test_final_answer_candidate_portfolio_groups_by_canonical_family() -> None:
    ...
    alpha = CandidateGenome(
        ...
        uncertainty_notes=["alpha uncertainty", "second note"],
            metadata={"nextgen": {"canonical_mechanism_family_id": "alpha#m1", "canonical_mechanism_family_version": "bounded-mechanism-v2"}},
        multihead_scores={"rank_score": 0.9},
    )
    beta = CandidateGenome(
        id="B1",
        concise_claim="beta direction",
            metadata={"nextgen": {"canonical_mechanism_family_id": "beta#m2", "canonical_mechanism_family_version": "bounded-mechanism-v2"}},
    ...
    assert "| rank | direction | validation status | uncertainty |" in text
...
def test_final_answer_candidate_status_is_answer_first_not_local_verified() -> None:
    ...
    assert "| 1 | local verified only | preliminary_passed |" in text

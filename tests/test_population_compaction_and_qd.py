...
def test_current_round_unevaluated_offspring_survive_until_next_evaluator_pass() -> None:
    old = [_candidate(f"old-{index}", niche="alpha", score=1.0) for index in range(4)]
    new = [_candidate(f"new-{index}", niche="alpha", score=0.0) for index in range(2)]
    for candidate in old:
        candidate.metadata["created_in_round"] = 1
    old[0].metadata["created_in_round"] = "model-authored-unknown"
    for candidate in new:
        candidate.metadata["created_in_round"] = 2
    population = CandidatePopulation([*old, *new])

    compact_live_population(
        population,
        ArchiveManager(),
        EvolutionPolicy(metadata={"live_bin_capacity": 1, "quality_diversity_rare_reserve_per_bin": 0}),
        branch_factor=1,
        round_index=2,
    )

    live_ids = {candidate.id for candidate in population.candidates}
    assert {"new-0", "new-1"}.issubset(live_ids)

    for candidate in population.candidates:
        if candidate.id.startswith("new-"):
            candidate.metadata["evaluator"] = {"status": "failed", "passed": False}
    next_round = compact_live_population(
        population,
        ArchiveManager(),
        EvolutionPolicy(metadata={"live_bin_capacity": 1, "quality_diversity_rare_reserve_per_bin": 0}),
        branch_factor=1,
    ...
    assert {"new-0", "new-1"}.intersection(next_round.compacted_clone_ids)

...
def test_protected_verification_regime_is_exact_even_when_over_budget() -> None:
    probe = "BEGIN-" + ("X" * 30_000) + "-END"
    payload = {
        "candidates": [{"id": "c", "artifact": "x"}],
        "contract": {"normalized_goal": "g"},
        "_prompt_context_controls": {
            "verification_regime": [
                {"id": "obl", "must_pass": True, "exogeneity_probe": {"content": probe}}
            ]
        },
    }

    view = build_prompt_view("nexus_critique_candidates", payload, max_chars=5_000)

    assert view.payload["verification_regime"][0]["exogeneity_probe"]["content"] == probe

...
def test_model_defined_search_planes_drive_project_seed_breadth_without_low_level_default() -> None:
    ...
    contract = NexusObjectiveContract(
        ...
        outcome_policy={
            "search_space_plan": {
                "candidate_families": [
                    {"id": "candidate lifecycle", "description": "candidate survival, repair, and parenthood"},
    ...
    assert {"candidate lifecycle", "search_distribution", "final_answer_boundary"}.issubset(niches)
    assert "candidate_lifecycle" not in niches

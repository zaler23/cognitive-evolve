...
from cognitive_evolve_runtime.ranking.relative_rater import RelativeRater, RelativeRankingResult, _verification_score, relative_rater_schema
...
def test_verification_score_orders_pass_unknown_and_failed() -> None:
    verified = CandidateGenome(id="verified", verification_result={"passed": True})
    missing = CandidateGenome(id="missing")
    unknown = CandidateGenome(id="unknown", verification_result={"diagnostics": ["not run"]})
    failed = CandidateGenome(id="failed", verification_result={"passed": False})

    assert _verification_score(verified) == 1.0
    assert _verification_score(missing) is None
    assert _verification_score(unknown) is None
    assert _verification_score(failed) == 0.25
    assert _verification_score(verified) > _verification_score(failed)


def test_verification_score_keeps_not_run_and_inconclusive_neutral() -> None:
    not_run = CandidateGenome(id="not-run", verification_result={"passed": False, "validation_status": "not_run"})
    inconclusive = CandidateGenome(id="inconclusive", verification_result={"passed": False, "validation_status": "inconclusive"})

    assert _verification_score(not_run) is None
    assert _verification_score(inconclusive) is None

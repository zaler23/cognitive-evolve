...
def test_no_binding_text_candidate_is_not_penalized_as_an_invented_source_claim(tmp_path: Path) -> None:
    _project(tmp_path)
    generic = CandidateGenome(id="generic", artifact={"answer": "pure in-memory result"}, artifact_type="answer")
    invented = CandidateGenome(
        id="invented",
        source_bindings=[{"path": "cognitive_evolve_runtime/missing.py", "symbol": "missing"}],
    )

    generic_manifest = annotate_candidate_source_bindings(generic, project_root=tmp_path)
    invented_manifest = annotate_candidate_source_bindings(invented, project_root=tmp_path)

    assert generic_manifest.binding_class == "no_binding"
    assert "selection_deprioritized_until_new_delta" not in generic.metadata
    assert invented_manifest.binding_class == "invented"
    assert invented.metadata["selection_deprioritized_until_new_delta"] is True

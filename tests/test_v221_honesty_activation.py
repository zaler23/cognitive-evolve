...
def test_candidate_metadata_cannot_self_certify_engine_probe_success() -> None:
    result = VerificationResult(
        passed=True,
        replayable=True,
        strength=VerificationStrength.EXECUTABLE,
        metadata={
            "oracle_kind": "executable",
            "actual_probe_verdicts": {"p": {"verdict_flipped": False}},
            "engine_known_good_bad_pair": True,
            "engine_survived_count": 4,
        },
    )
    regime = GroundingRegime(
        regime_id="r",
    ...
    observed = execute_probes(result, regime)
    measured = measure_honesty(result, regime, observed, {"frozen_artifact_hash": "a", "verifier_fingerprint": "v", "replay_verified": True})

    assert observed.get("p") is None
    assert measured.exogeneity_score == 0.0
    assert measured.variety_score == 0.0
    assert measured.falsification_score == 0.0
...
def test_replay_runner_requires_replayable_oracle() -> None:
    ...
    exec_result = VerificationResult(passed=True, replayable=True, metadata={"oracle_kind": "executable", "diagnostics_only": False, "replay_verified": True})
    assert build_replay_record(candidate, exec_result, verifier_fingerprint="vf", oracle_kind="executable")["replay_verified"] is True


def test_declared_toolrunner_obligation_cannot_self_certify_executable() -> None:
    candidate = CandidateGenome(id="T2", artifact="plain prose", concise_claim="plain prose")
    records = run_obligations_for_population(
        [candidate],
        [{"id": "obl", "oracle_kind": "toolrunner", "replayable": True, "diagnostic_matcher": "absent", "falsification_budget": {"count": 4}}],
        cache={},
    )
    result = VerificationResult.from_dict(records[0]["verification_result"])

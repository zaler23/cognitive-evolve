...
class QuotaSynthModel(FakeNexusModel):
    def synthesize_result(self, *, population: list[CandidateGenome], archives: Any, contract: Any, world: Any) -> dict[str, Any]:
        self.calls.append("synthesize_result")
        raise LLMResponseError("429 resource_exhausted provider quota")
...
def test_exhaustive_model_tier_controls_nexus_rounds_without_explicit_rounds(monkeypatch, tmp_path) -> None:
    ...
    assert "plan_mutations" not in fake.calls
    assert "synthesize_result" in fake.calls


def test_final_synthesis_quota_error_pauses_instead_of_local_fallback(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("COGEV_NEXUS_PROFILE_EXHAUSTIVE_SAFETY_ROUNDS", "1")
    fake = QuotaSynthModel()
    with _temporary_model_runtime("cognitive-evolve-one-shot-exhaustive"):
        result = EngineOrchestrator(model=fake).run(
            "Hard problem",
            context={
    ...
    assert "model_synthesis_local_fallback" not in " ".join(result.evolution["synthesis"]["warnings"])

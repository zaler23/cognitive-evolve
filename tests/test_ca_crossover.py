...
from cognitive_evolve_runtime.candidates.crossover import crossover, descriptor_tokens, jaccard_similarity, neighborhood_crossover_partner
...
def test_mutation_child_does_not_inherit_current_verification_state() -> None:
    parent = _candidate("P", "verified parent", niche="math")
    parent.tool_results = [{"tool": "pytest", "status": "passed"}]
    parent.verification_trace = [{"passed": True, "evidence_ref": "old"}]
    parent.verification_result = {"passed": True, "score": 1.0}
    parent.evidence_delta = {"added": ["source"], "verified": ["old-proof"], "refuted": ["old-refute"]}

    child = MutationEngine().mutate(parent, MutationPlan(operator=MutationOperator.DEEPEN, instruction="change artifact"))

    assert child.tool_results == []
    assert child.verification_trace == []
    assert child.verification_result == {}
    assert child.evidence_delta["added"] == ["source"]
    assert "verified" not in child.evidence_delta
    assert child.metadata["parent_verification_summary"]["verification_result"]["passed"] is True


def test_crossover_child_does_not_merge_parent_verification_state() -> None:
    parent_a = _candidate("A", "a", niche="math")
    parent_b = _candidate("B", "b", niche="math")
    parent_a.tool_results = [{"tool": "pytest", "status": "passed"}]
    parent_b.verification_trace = [{"passed": True, "evidence_ref": "b-proof"}]
    parent_a.verification_result = {"passed": True, "score": 1.0}
    parent_b.evidence_delta = {"verified": ["b-proof"], "added": ["source-b"]}
    ...
    assert child.verification_trace == []
    assert child.verification_result == {}
    assert child.evidence_delta["added"] == ["source-b"]
    assert "verified" not in child.evidence_delta
    assert child.metadata["parent_verification_summary"]["A"]["verification_result"]["passed"] is True
    assert child.metadata["parent_verification_summary"]["B"]["verification_trace_count"] == 1


def test_project_patch_crossover_child_does_not_merge_parent_verification_state() -> None:
    parent_a = ProjectCandidateGenome(id="PA", patch_set=[PatchOperation(path="a.py", operation="write", content="a")])
    parent_b = ProjectCandidateGenome(id="PB", patch_set=[PatchOperation(path="b.py", operation="write", content="b")])
    parent_a.verification_result = {"passed": True}
    parent_b.verification_trace = [{"passed": True}]
    parent_b.evidence_delta = {"verified": ["old"], "added": ["source"]}
    ...
    assert child.evidence_delta["added"] == ["source"]
    assert "verified" not in child.evidence_delta
    assert child.metadata["parent_verification_summary"]["PA"]["verification_result"]["passed"] is True
    assert child.metadata["parent_verification_summary"]["PB"]["verification_trace_count"] == 1

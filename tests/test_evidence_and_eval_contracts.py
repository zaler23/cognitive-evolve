...
from cognitive_evolve_runtime.evaluators.evidence import EvidenceRecord, apply_evidence_record, evidence_final_blocked, evidence_parent_blocked
...
def test_evidence_blocked_helpers_read_canonical_state() -> None:
    blocked = CandidateGenome(id="blocked", metadata={"evidence_state": {"final_blocked": True, "parent_blocked": True}})
    missing = CandidateGenome(id="missing")

    assert evidence_final_blocked(blocked) is True
    assert evidence_parent_blocked(blocked) is True
    assert evidence_final_blocked(missing) is False
    assert evidence_parent_blocked(missing) is False


def test_final_evidence_without_hash_does_not_carry_forward_to_new_probe() -> None:
    old_final = EvidenceRecord(
        candidate_id="C1",
        stage="final",
        final_blocked=False,
        score=1.0,
        confidence=1.0,
        metadata={"authority": "final"},
    )
    new_probe = EvidenceRecord(
        candidate_id="C1",
        stage="probe",
        final_blocked=True,
        metadata={"authority": "probe", "artifact_hash": "new-artifact"},
    )

    state = aggregate_evidence_state([old_final, new_probe])

    assert state["final_blocked"] is True


def test_final_evidence_with_matching_hash_can_carry_forward_to_new_probe() -> None:
    old_final = EvidenceRecord(
        candidate_id="C1",
        stage="final",
        final_blocked=False,
        score=1.0,
        confidence=1.0,
        metadata={"authority": "final", "artifact_hash": "same-artifact"},
    )
    new_probe = EvidenceRecord(
        candidate_id="C1",
        stage="probe",
        final_blocked=True,
        metadata={"authority": "probe", "artifact_hash": "same-artifact"},
    )
...
def test_stale_terminal_reject_does_not_carry_forward_to_new_artifact() -> None:
    old_certificate = EvidenceRecord(
        candidate_id="C1",
        stage="final",
        terminal_reject=True,
        metadata={"authority": "certificate", "artifact_hash": "old-artifact"},
    )
    new_probe = EvidenceRecord(
        candidate_id="C1",
        stage="probe",
    ...
    state = aggregate_evidence_state([old_certificate, new_probe])
    candidate = CandidateGenome(id="C1", metadata={"evidence_records": [old_certificate.to_dict()]})
    apply_evidence_record(candidate, new_probe)

    assert state["terminal_reject"] is False
    assert state["authority"] == "probe"
    assert candidate.metadata["terminal_failure"] is False


def test_same_artifact_terminal_reject_still_carries_forward() -> None:
    old_certificate = EvidenceRecord(
        candidate_id="C1",
        stage="final",
        terminal_reject=True,
        metadata={"authority": "certificate", "artifact_hash": "same-artifact"},
    )
    new_probe = EvidenceRecord(
        candidate_id="C1",
        stage="probe",
    ...
    assert state["terminal_reject"] is True
    assert state["authority"] == "certificate"
...
def test_evidence_planner_adapters_executor_and_store(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert safe_payload["query_present"] is True
    assert "query" not in safe_payload
    assert "secret" not in safe_payload["context"]
    assert "workspace_root" not in safe_payload["context"]
    ...
    original_private_check = ep.HTTPJsonEvidenceAdapter._is_private_or_local_address
    monkeypatch.setattr(ep.HTTPJsonEvidenceAdapter, "_resolve_endpoint_addresses", lambda self, host, port: {"resolver-ok"})
    monkeypatch.setattr(
        ep.HTTPJsonEvidenceAdapter,
        "_is_private_or_local_address",
        lambda self, address: False if address == "resolver-ok" else original_private_check(self, address),
    )

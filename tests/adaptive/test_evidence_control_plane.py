...
def test_external_evaluator_is_authoritative_for_native_text_without_artifact_policy() -> None:
    artifact = "NATIVE-SYNTAX/1\nalpha => beta"
    candidate = CandidateGenome(id="native-pass", artifact=artifact, artifact_type="native_syntax")
    ...
    record = ProgressiveEvaluator().evaluate_result(
        ...
        EvaluatorResult(
            ...
            passed=True,
            metrics={"score": 0.8},
            diagnostics=["raw_score:0.80000000"],
        ),
        spec=EvaluatorSpec(),
    )
    ...
    assert record.metadata["artifact_state"]["status"] == "clean"
    assert record.metadata["artifact_state"]["normalized_artifact"] == artifact
    assert record.metadata["artifact_state"]["schema_cleanliness"] == 1.0
    assert record.metadata["artifact_normalizer_audit"]["status"] == "refolded"
    assert record.metadata["artifact_normalizer_audit"]["diagnostics"] == ["artifact_not_machine_parseable"]
    assert record.metadata["artifact_hash"]
    assert record.diagnostics == []
    assert record.hints == []
    assert record.emitted_challenge_ids == []
    assert record.metadata["challenge_items"] == []
    assert record.final_blocked is True
    assert candidate.multihead_scores["schema_cleanliness"] == 1.0
    assert memory.compile_search_pressure() is None


def test_failed_native_text_keeps_only_external_failure_pressure_without_artifact_policy() -> None:
    candidate = CandidateGenome(id="native-fail", artifact="NATIVE-SYNTAX/1\nbroken", artifact_type="native_syntax")
    ...
    record = ProgressiveEvaluator().evaluate_result(
        ...
        EvaluatorResult(
            ...
            passed=False,
            metrics={"score": 0.2},
            diagnostics=["native_rule_rejected"],
        ),
        spec=EvaluatorSpec(),
    )

    assert "native_rule_rejected" in record.diagnostics
    assert all("machine_parse" not in item for item in [*record.diagnostics, *record.hints])
    assert all(item["metadata"]["category"] != "machine_parse_failure" for item in record.metadata["challenge_items"])
    assert record.metadata["artifact_state"]["schema_cleanliness"] == 1.0
    assert record.metadata["artifact_normalizer_audit"]["status"] == "refolded"
    assert record.final_blocked is True


def test_explicit_machine_artifact_policy_keeps_native_text_schema_pressure() -> None:
    candidate = CandidateGenome(id="native-machine", artifact="NATIVE-SYNTAX/1\nalpha => beta", artifact_type="native_syntax")
    spec = EvaluatorSpec.from_mapping({"evidence": {"machine_artifact_required": True}})

    record = ProgressiveEvaluator().evaluate_result(
        candidate,
        EvaluatorResult(candidate_id=candidate.id, status="passed", passed=True, metrics={"score": 0.8}, diagnostics=[]),
        spec=spec,
    )

    assert record.metadata["artifact_state"]["status"] == "malformed"
    assert "artifact_not_machine_parseable" in record.diagnostics
    assert record.emitted_challenge_ids
    assert any(item["metadata"]["category"] == "machine_parse_failure" for item in record.metadata["challenge_items"])


def test_explicit_clean_artifact_policy_keeps_existing_evaluator_diagnostics() -> None:
    candidate = CandidateGenome(id="machine-clean", artifact={"answer": 1}, artifact_type="machine")
    spec = EvaluatorSpec.from_mapping({"evidence": {"machine_artifact_required": True}})
    ...
    assert record.diagnostics == ["raw_score:0.80000000"]
    assert record.hints == ["repair diagnostic: raw_score:0.80000000"]
...
def test_final_projection_returns_answer_without_internal_directives() -> None:
    ...
    assert "Best current candidate" in markdown
    assert "boundary case failed" not in markdown
    assert projection.to_dict()["artifact"] == {"answer": 1}
...
def test_final_projection_allows_failed_best_current_direction() -> None:
    ...
    assert projection.best_current_direction["validation_status"] == "preliminary_failed"

...
@pytest.mark.parametrize(
    ...
    [
        "pass\n",
        "print('not-json')\n",
        "import json\nprint(json.dumps({'passed': 'false', 'metrics': {'correctness': True}}))\n",
    ],
)
def test_external_evaluator_requires_explicit_boolean_or_status_verdict(tmp_path: Path, body: str) -> None:
    evaluator = tmp_path / "evaluator.py"
    ...
    result = ExternalEvaluatorRunner().evaluate_candidate(CandidateGenome(id="C-invalid"), spec=spec)

    assert result.status == "invalid_evaluator_output"
    assert result.passed is False
    assert result.metrics["correctness"] is False
    assert "invalid_evaluator_output" in result.diagnostics
...
def test_final_certificate_blocks_model_solved_when_evaluator_failed() -> None:
    ...
    assert certificate["preliminary_validation_passed"] is False
    assert "preliminary_evaluator_failed_advisory" in certificate["blocking_reasons"]
    assert updated["objective_solved"] is False
    assert updated.get("critical_failures", []) == []


def test_configured_evaluator_pass_is_a_bound_preliminary_pass() -> None:
    candidate = CandidateGenome(id="C1", artifact="answer")
    candidate.metadata["evaluator"] = {"status": "passed", "passed": True}

...
def _public_paths() -> list[Path]:
    try:
        ...
        return [path for path in ROOT.rglob("*") if not any(part in SKIP_DIR_NAMES for part in path.relative_to(ROOT).parts)]
    paths: set[Path] = set()
...
def test_public_tree_has_no_cache_runtime_or_backup_artifacts() -> None:
    ...
    for path in _public_paths():
        ...
        elif path.is_dir() and rel in FORBIDDEN_ROOT_DIR_NAMES:
            offenders.append(rel + "/")
        elif path.is_dir() and path.name.startswith("llm-calls"):
            offenders.append(rel + "/")
        elif path.is_file():
            if path.name in FORBIDDEN_FILE_NAMES or path.suffix in FORBIDDEN_SUFFIXES:
                offenders.append(rel)
            if path.name.startswith("llm-call-ledger") and path.name.endswith(".jsonl"):
                offenders.append(rel)
...
def test_public_api_launcher_avoids_project_local_venv_and_implicit_port_kill() -> None:
    ...
    assert "load_env_file" in launcher
    assert "(3, 11) <= sys.version_info[:2] < (3, 14)" in launcher
    assert "Python >=3.11 and <3.14 is required." in launcher
    assert "3." + "10" not in launcher
    assert "3." + "15" not in launcher

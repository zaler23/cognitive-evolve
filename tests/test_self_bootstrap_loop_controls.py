...
def test_large_seed_pool_frontier_is_advisory_before_observed_outcomes() -> None:
    ...
    assert trace["advisory_unselected_count"] == 6
    assert len(candidates) == 10
    assert all(candidate.current_fate == CandidateFate.ACTIVE.value for candidate in candidates)
...
def test_policy_update_preserves_seed_search_kernel_metadata_from_model_policy() -> None:
    ...
    diagnosis = SearchDiagnosis(stagnation_detected=True, stagnation_type="DiversityCollapse")
...
def test_policy_update_preserves_or_replaces_active_search_space_without_merging() -> None:
    previous_plan = {
        "source": "model_authored_search_space",
        "families": [{"id": f"previous_{index}", "plane": f"plane_{index}"} for index in range(8)],
    }
    replacement_plan = {
        "source": "model_policy_update",
        "families": [{"id": "replacement", "plane": "new"}],
    }
    diagnosis = SearchDiagnosis(stagnation_detected=True, stagnation_type="DiversityCollapse")

    class _PartialPolicyModel:
        def update_policy(self, *, policy: EvolutionPolicy, diagnosis: SearchDiagnosis) -> dict:
            return EvolutionPolicy(metadata={"updated_by_model": True}).to_dict()

    class _ReplacementPolicyModel:
        def update_policy(self, *, policy: EvolutionPolicy, diagnosis: SearchDiagnosis) -> dict:
            return EvolutionPolicy(search_space=replacement_plan).to_dict()

    preserved = PolicyUpdater().update(
        EvolutionPolicy(search_space=previous_plan),
        diagnosis,
        model=_PartialPolicyModel(),
    )
    replaced = PolicyUpdater().update(
        EvolutionPolicy(search_space=previous_plan),
        diagnosis,
        model=_ReplacementPolicyModel(),
    )
    still_empty = PolicyUpdater().update(EvolutionPolicy(), diagnosis, model=_PartialPolicyModel())
    restored = EvolutionPolicy.from_json(preserved.to_json())

    assert preserved.search_space == previous_plan
    assert restored.search_space == previous_plan
    assert restored.policy_hash == preserved.policy_hash
    assert replaced.search_space == replacement_plan
    assert still_empty.search_space == {}


def test_policy_update_keeps_model_led_adaptation_on_healthy_continue_diagnosis() -> None:
    class _PolicyModel:
        ...
        def update_policy(self, *, policy: EvolutionPolicy, diagnosis: SearchDiagnosis) -> dict:
            ...
            return EvolutionPolicy(
                search_space={"families": [{"id": "adapted"}]},
                metadata={"updated_by_model": True},
            ).to_dict()

    model = _PolicyModel()
    policy = EvolutionPolicy(search_space={"families": [{"id": "initial"}]}, metadata={"preserved": True})

    updated = PolicyUpdater().update(
        policy,
        SearchDiagnosis(stagnation_detected=False, stagnation_type="None", recommended_actions=["continue"]),
        model=model,
    )

    assert model.calls == 1
    assert updated.search_space == {"families": [{"id": "adapted"}]}
    assert updated.metadata["updated_by_model"] is True
    assert updated.updated_from_diagnoses[-1] == "None"

    class _UnchangedPolicyModel:
        def update_policy(self, *, policy: EvolutionPolicy, diagnosis: SearchDiagnosis) -> dict:
            return policy.to_dict()

    before_hash = hashlib.sha256(json.dumps(policy.search_space, sort_keys=True).encode()).hexdigest()
    unchanged = PolicyUpdater().update(
        policy,
        SearchDiagnosis(stagnation_detected=False, stagnation_type="None", recommended_actions=["continue"]),
        model=_UnchangedPolicyModel(),
    )
    after_hash = hashlib.sha256(json.dumps(unchanged.search_space, sort_keys=True).encode()).hexdigest()

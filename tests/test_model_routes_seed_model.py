...
def test_seed_route_isolated_from_default_model(tmp_path) -> None:
    ...
    for expected in {"build_objective_contract", "build_evolution_policy", "relative_rank", "diagnose_search_state", "generate_offspring", "synthesize_result"}:
        assert expected in default.calls
    assert "plan_mutations" not in default.calls
    assert result.evolution["budget_history"][0]["generation_plan"]["mutation_plan_source"] == "runtime_lineage_envelope"

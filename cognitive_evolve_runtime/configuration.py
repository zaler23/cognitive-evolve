...
def load_layered_config(path: Path | str | None = None, *, override: bool = False) -> Path | None:
    ...
    The parser supports the simple YAML subset used by documented config files:
    indentation-based nested mappings, scalar values, and ``${ENV_VAR}``
    references.
    """
...
def config_resolution_diagnostics() -> dict[str, Any]:
    ...
    sources = [
        {
            "path": redact_text(str(source.path)),
            "kind": source.kind,

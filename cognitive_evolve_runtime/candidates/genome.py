...
class CandidateGenome:
    ...
    def extract_inheritable_gene_summary(self) -> str:
        ...
        parts.extend(self.edge_knowledge_seeds)
        parts.extend(self.failure_lessons)
    ...
    def from_dict(cls, data: dict[str, Any]) -> "CandidateGenome":
        return cls(
            ...
            verification_result=coerce_dict(data.get("preliminary_result")) or coerce_dict(data.get("verification_result")),
...
def _producer_safe_candidate_payload(value: Any, *, preliminary_root: bool = False) -> Any:
    """Serialize machine claim fields as preliminary while preserving prose."""

    if isinstance(value, list):
        return [_producer_safe_candidate_payload(item) for item in value]
    if not isinstance(value, dict):
        return value
    out: dict[str, Any] = {}
    for raw_key, raw_value in value.items():
        key = str(raw_key)
        if key in {"artifact", "answer", "content", "old_text", "new_text", "raw_output", "raw_response", "formal_artifacts"}:
            out[key] = raw_value
            continue
        target = key
        child_preliminary = False
        if key in {"verification_result", "verified_result"}:
            target = "preliminary_result"
            child_preliminary = True
        elif key == "verification_status":
            target = "validation_status"
            raw_value = _preliminary_status(raw_value)
        elif key == "validation_status":
            raw_value = _preliminary_status(raw_value)
        elif key == "objective_solved":
            raw_value = False
        elif key == "solved" and isinstance(raw_value, bool):
            raw_value = False
        elif key == "completion_status" and str(raw_value or "").strip().lower() == "solved":
            raw_value = "completed"
        elif key == "stop_reason" and str(raw_value or "").strip().lower() in {"solved", "objective_solved"}:
            raw_value = "candidate_ready_for_external_review"
        if preliminary_root and key == "status":
            raw_value = _preliminary_status(raw_value)
        out[target] = _producer_safe_candidate_payload(raw_value, preliminary_root=child_preliminary)
    if preliminary_root and "validation_status" not in out:
        status = str(out.get("status") or "")
        if status in {"not_run", "preliminary_passed", "preliminary_failed", "inconclusive"}:
            out["validation_status"] = status
        elif out.get("passed") is True:
            out["validation_status"] = "preliminary_passed"
        elif out.get("passed") is False:
            out["validation_status"] = "preliminary_failed"
        else:
            out["validation_status"] = "inconclusive"
    return out


def _preliminary_status(value: Any) -> str:
    text = str(value or "").strip().lower()
    if text in {"verified", "pass", "passed", "success", "ok", "preliminary_pass"}:
        return "preliminary_passed"
    if text in {"fail", "failed", "error", "preliminary_fail"}:
        return "preliminary_failed"
    if text in {"not_run", "preliminary_passed", "preliminary_failed", "inconclusive"}:
        return text
    return str(value or "inconclusive")

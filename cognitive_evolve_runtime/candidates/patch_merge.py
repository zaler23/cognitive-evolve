...
def project_patch_crossover(parent_a: ProjectCandidateGenome, parent_b: ProjectCandidateGenome, *, instruction: str = "combine complementary project patch genes") -> ProjectCandidateGenome:
    ...
    return ProjectCandidateGenome(
        ...
        metadata={
            "crossover_instruction": instruction,
            "patch_merge_conflicts": [c.to_dict() for c in merge.conflicts],
            "parent_verification_summary": _parent_verification_summary(parent_a, parent_b),
        },
)
...
def _parent_verification_summary(*parents: ProjectCandidateGenome) -> dict[str, Any]:
    summary: dict[str, Any] = {}
    for parent in parents:
        item: dict[str, Any] = {}
        if parent.verification_result:
            item["verification_result"] = dict(parent.verification_result)
        if parent.verification_trace:
            item["verification_trace_count"] = len(parent.verification_trace)
        if parent.tool_results:
            item["tool_result_count"] = len(parent.tool_results)
        if item:
            summary[parent.id] = item
    return summary

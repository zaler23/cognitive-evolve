...
def _verdict(
    *,
    ...
    canonical_blockers = [re.sub(r"\s+", " ", str(item or "")).strip() for item in blockers if str(item or "").strip()]
    canonical_targets = [re.sub(r"\s+", " ", str(item or "")).strip() for item in repair_targets if str(item or "").strip()]
    canonical_diagnostics = [str(item) for item in diagnostics if str(item or "").strip()]
    signature = _signature(category, canonical_blockers, canonical_targets)
    return FailureVerdict(
        ...
        repair_targets=canonical_targets,
        ...
        out.append(
            {
                ...
                "source_bindings": [{"path": path, "kind": "source_file", "source": "failure_classifier"} for path in targets],
...
def _next_action_for_blocker(blocker: str, *, category: str, targets: list[str]) -> str:
    ...
    if category == "repairable_patch_syntax_or_context" or any(token in lowered for token in ("unified_patch_failed", "malformed patch", "hunk", "eof", "old_text")):
        target_text = ", ".join(targets) or "the declared patch targets"
    ...
    if category == "repairable_static_or_syntax_failure":
        target_text = ", ".join(targets) or "the changed files"
...
def _signature(category: str, blockers: list[str], targets: list[str]) -> str:
    return stable_hash({"category": category, "blockers": blockers, "targets": targets})

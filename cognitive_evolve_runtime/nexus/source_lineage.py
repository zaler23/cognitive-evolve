    ...
    if exists:
        if symbol and not symbol_exists:
            if touches and defines:
                ...
                diagnostics.append("patch_target_missing" if not touches else "declared_new_symbol_not_created" if mode == "existing_file_extension_unmaterialized" else "source_binding_missing_symbol")
        else:
            mode = "existing_file_refinement"
    else:
...
def allowed_materialization_path(path: str, *, materialization_scope: list[str] | tuple[str, ...] | None = None) -> bool:
    normalized = _normalize_project_scope_path(path)
    if not normalized or normalized.startswith(("/", "../")) or "/../" in normalized:
        return False
    scopes = tuple(
        scope
        for scope in (_normalize_project_scope_path(item, directory=True) for item in (materialization_scope or ALLOWED_MATERIALIZATION_PREFIXES))
        if scope
    )
    if not scopes:
        return False
...
def _normalize_project_scope_path(value: Any, *, directory: bool = False) -> str:
    text = str(value or "").strip().replace("\\", "/")
    while text.startswith("./"):
        text = text[2:]
    if not text or text in {".", ".."} or text.startswith(("/", "../")) or "/../" in text or "://" in text:
        return ""

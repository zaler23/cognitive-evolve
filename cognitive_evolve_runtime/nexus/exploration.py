"""Task-neutral exploration helpers for explicit deterministic Nexus runs.

The templates preserve search breadth across direct routes, counter-routes,
tool-grounded fragments, and high-variance edge ideas before ranking narrows the
pool.  They are encoded as CandidateGenome search moves and remain seeds for
evolution, not claims of truth.
"""
...
def amplify_population(
    *,
    ...
    """Expand a deterministic population to the requested exploration breadth.

    Existing candidates are never replaced. Structured search seeds are added
    only when the explicit deterministic path needs more breadth. Supplemental
    candidates are marked ``search_seed_not_final`` for
    critique, prompt views, metadata inheritance, and journaling; final synthesis
    does not read this marker as an eligibility gate.
    """
    ...
    for template in templates:
        if len(candidates) >= minimum:
            break
        template_key = template.niche.strip().lower().replace(" ", "_")
        if template_key in used_keys or template.seed_type.lower() in used_keys:
            continue
...
def action_palette_for_round(round_index: int, diagnosis_actions: Iterable[str] | None = None) -> list[str]:
    """Blend diagnosis actions with a domain-neutral variation palette."""
...
def _candidate_key(candidate: CandidateGenome) -> str:
    if candidate.niche_memberships:
        return str(candidate.niche_memberships[0]).strip().lower().replace(" ", "_")

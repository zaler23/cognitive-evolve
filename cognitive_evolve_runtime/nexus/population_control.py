...
def compact_live_population(
    ...
    *,
    ...
    awaiting_evaluation_ids: set[str] = set()
    for candidate in population.candidates:
        metadata = candidate.metadata if isinstance(candidate.metadata, dict) else {}
        try:
            created_in_round = int(metadata.get("created_in_round"))
        except (TypeError, ValueError):
            created_in_round = -1
        awaiting_next_evaluation = (
            created_in_round == int(round_index or 0)
            and not isinstance(metadata.get("evaluator"), dict)
        )
        if awaiting_next_evaluation:
            awaiting_evaluation_ids.add(candidate.id)
    ...
    preliminary_incumbent = select_preliminary_incumbent(live)
    if preliminary_incumbent is not None:
        protected_ids.add(preliminary_incumbent.id)
    protected: list[CandidateGenome] = []
    qd_input: list[CandidateGenome] = []
    for candidate in live:
        fate = CandidateFate.normalize(candidate.current_fate)
        awaiting_next_evaluation = candidate.id in awaiting_evaluation_ids
        if candidate.id in protected_ids or awaiting_next_evaluation or (fate == CandidateFate.DORMANT.value and archives.is_final_answer_eligible(candidate)):

...
def classify_candidate(candidate: dict[str, Any], search_space_map: dict[str, Any], *, objective: str = "") -> dict[str, Any]:
    ...
    if family_id not in known and families:
        family_id = "singleton_" + stable_hash({"candidate": text, "declared_family": family_id})[:12]
...
def _normalize_model_families(values: list[Any]) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(values, start=1):
        ...
        family_id = raw_id or f"model_defined_plane_{index}"
        family = dict(item)
        family["id"] = family_id
...
def _slug(value: str) -> str:
    ...
    return text if len(text) <= 320 else text[:300] + "_" + stable_hash(text)[:16]

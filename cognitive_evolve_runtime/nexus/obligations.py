...
This module deterministically computes proof-progress signals for deduplication
and search pressure. It never proves the theorem or owns final acceptance:
for proof-like objectives, narrative deepening is not progress unless the
candidate carries a concrete formal object and a ledger delta against named
obligations.
"""
...
def formal_artifact_kind(artifact: dict[str, Any]) -> str:
    ...
    Accepted model payloads may use ``kind`` or JSON-schema-style ``type``.
    Treating ``type`` as an alias prevents valid

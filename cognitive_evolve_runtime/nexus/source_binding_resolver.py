...
commands, but engine resolution owns source-dependent archive and repair
routing. Unresolved or invented bindings remain repair targets, never count as
implementation evidence, and do not themselves own final acceptance.
"""
...
def annotate_candidate_source_bindings(candidate: CandidateGenome, *, project_root: str | Path | None = None) -> SourceBindingManifest:
    ...
    if manifest.binding_class in {"invented", "unresolved"}:
...
def _patch_materializes(candidate: CandidateGenome | None, relative_path: str, symbol: str = "") -> bool:
    ...
    path_ok = relative_path in applied_files
    ...
    if symbol and symbol not in text:
        return False
    return bool(path_ok or (relative_path and relative_path in text))

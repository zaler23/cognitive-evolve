...
class SynthesizedResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        data = asdict(self)
        data["status"] = _public_completion_status(data.get("status"))
        data["completion_status"] = _public_completion_status(data.get("completion_status"))
        data["objective_solved"] = False
        closure = dict(data.get("closure_certificate") or {})
        ...
        if isinstance(final_certificate, dict):
            ...
            final_certificate["objective_solved"] = False
            closure["final_certificate"] = final_certificate
        data["closure_certificate"] = closure
        return data
...
class FinalSynthesizer:
    ...
    def synthesize(self, *, population: CandidatePopulation, archives: ArchiveManager, contract: Any | None = None, world: Any | None = None) -> SynthesizedResult:
        ...
        if preliminary_incumbent is not None:
            return SynthesizedResult(
                status="preliminary_evaluator_selected",
                final_answer=_candidate_answer_text(preliminary_incumbent),
                best_candidate_id=preliminary_incumbent.id,
                archives_summary=archives.summary(),
                warnings=["final_artifact_bound_to_preliminary_evaluator_incumbent"],
                objective_solved=False,
                answer_produced=True,
                best_current_direction=best_current_direction_payload(
                    preliminary_incumbent,
                    route=_answer_route(preliminary_incumbent),
                    contract=contract,
                ),
            )
        ...
        if best is None:
            best = _best_current_final_fallback_candidate(population.candidates, contract=contract)
            if best is not None:
                warnings.append("answer_first_used_best_current_direction")
        auxiliary = _best_auxiliary(archives, population=population)
        if best is None:
            repair_direction = select_best_current_direction(population.candidates, contract=contract)
            if repair_direction is not None and _candidate_blocked_until_repaired(repair_direction):
                warnings.append("final_answer_blocked_until_repaired")
                return SynthesizedResult(
                    status="route_incomplete",
                    final_answer="No final-answer eligible candidate emerged; the strongest current patch candidate is blocked until repaired.",
                    best_auxiliary_candidate_id=auxiliary.id if auxiliary else "",
                    archives_summary=archives.summary(),
                    warnings=warnings,
                    failure_analysis="A patch candidate exists, but patch applicability blocked it from final-answer fallback.",
                    best_current_direction=best_current_direction_payload(repair_direction, route="best_current", contract=contract),
                )
            return SynthesizedResult(
    ...
    def _model_synthesis(self, *, population: CandidatePopulation, archives: ArchiveManager, contract: Any | None, world: Any | None) -> SynthesizedResult | None:
        ...
        try:
            ...
            if _checkpointable_model_synthesis_error(exc):
                raise
        ...
        requested_best_current_only = False
        if requested_candidate_id:
            requested = candidate_by_id.get(requested_candidate_id)
            if requested is None:
                record_fallback(stage="final_synthesis", reason="unknown_candidate_id", target=requested_candidate_id)
                self._model_fallback_warning = "model_synthesis_local_fallback:unknown_candidate_id"
                return None
            if structurally_blocked(requested):
                record_fallback(stage="final_synthesis", reason="ineligible_candidate_id", target=requested_candidate_id)
                self._model_fallback_warning = "model_synthesis_local_fallback:ineligible_candidate_id"
                return None
            if _candidate_blocked_until_repaired(requested):
                raw_status = str(raw.get("status") or "").strip().lower().replace("-", "_")
                if raw_status in {"incomplete", "route_incomplete", "best_current_route"}:
                    warnings.append("model_synthesis_requested_candidate_best_current_only")
                    selected = requested
                    requested_best_current_only = True
                else:
                    record_fallback(stage="final_synthesis", reason="blocked_until_repaired", target=requested_candidate_id)
                    self._model_fallback_warning = "model_synthesis_local_fallback:blocked_until_repaired"
                    return None
            elif _final_answer_candidate_eligible(requested):
                selected = requested
            else:
                if _candidate_answer_text(requested).strip():
                    warnings.append("model_synthesis_requested_candidate_unbound_ineligible_candidate_id")
                else:
                    warnings.append("model_synthesis_requested_candidate_unbound_empty_answer_material")
                selected = None
            ...
            if selected is not None:
                ...
                selected = _best_current_final_fallback_candidate(population.candidates, contract=contract)
                if selected is not None:
                    warnings.append("model_synthesis_without_candidate_id_used_best_current_direction")

        selected_id = selected.id if selected is not None and not requested_best_current_only else ""
        if selected is not None and ("intent_directness" in raw or "intent_alignment_rationale" in raw):
            ...
            if raw.get("intent_alignment_rationale"):
                binding["alignment_rationale"] = str(raw.get("intent_alignment_rationale") or "")
        ...
        if selected is not None and _normalize_answer_text(final) != _normalize_answer_text(_candidate_answer_text(selected)):
            if requested_candidate_id:
                warnings.append("model_final_answer_differs_from_selected_candidate_artifact")
            else:
...
def _runtime_answer_candidate(archives: ArchiveManager, candidates: list[CandidateGenome]) -> CandidateGenome | None:
    ...
    if best is not None and _final_answer_candidate_eligible(best):
        return best
    eligible = [candidate for candidate in candidates if archives.is_final_answer_eligible(candidate) and _final_answer_candidate_eligible(candidate)]
    if eligible:
...
def _best_answer_candidate(candidates: list[CandidateGenome], *, contract: Any | None = None) -> CandidateGenome | None:
    ...
    if not eligible:
        return None
    return max(eligible, key=lambda candidate: (_answer_candidate_score(candidate, eligible, contract=contract) + measured_verification_bonus(candidate), candidate.id))


def _best_current_final_fallback_candidate(candidates: list[CandidateGenome], *, contract: Any | None = None) -> CandidateGenome | None:
    return select_best_current_direction([candidate for candidate in candidates if not _candidate_blocked_until_repaired(candidate)], contract=contract)
...
def _final_answer_candidate_eligible(candidate: CandidateGenome) -> bool:
    ...
    if fate in {CandidateFate.CULLED.value, CandidateFate.FAILED.value, CandidateFate.INCUBATING.value}:
        return False
    metadata = candidate.metadata if isinstance(candidate.metadata, dict) else {}
    if metadata.get("terminal_failure") or metadata.get("terminal_reject_reason") or metadata.get("final_answer_blocked_until_repaired"):
        return False
    return True


def _candidate_blocked_until_repaired(candidate: CandidateGenome) -> bool:
    fate = CandidateFate.normalize(getattr(candidate, "current_fate", ""))
    metadata = candidate.metadata if isinstance(candidate.metadata, dict) else {}
    return fate == CandidateFate.INCUBATING.value or bool(metadata.get("final_answer_blocked_until_repaired"))
...
def _unbound_best_current_payload(final_answer: str) -> dict[str, Any]:
    ...
    return {
        ...
        "external_review_required_reason": "answer_unbound_to_candidate_artifact",
    }
...
def _checkpointable_model_synthesis_error(exc: Exception) -> bool:
    if str(exc).startswith("nexus_synthesize_result structured prompt could not fit effective cap"):
        return False
...
def _public_completion_status(value: Any) -> str:
    text = str(value or "completed").strip()
    return "completed" if text.lower() == "solved" else text or "completed"

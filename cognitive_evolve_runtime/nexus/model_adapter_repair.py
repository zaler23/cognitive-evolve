...
def _repair_dynamic_artifact_contract(data: dict[str, Any], *, user_goal: str) -> dict[str, Any]:
    ...
    if isinstance(dac_value, dict):
        ...
        if dac is not None:
            repaired["dynamic_artifact_contract"] = {**dac_value, **dac.to_dict()}
    else:
...
def _repair_project_contract_paths(repaired: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    basenames that do not exist in the project snapshot.  Those names are useful
    intent signals, but letting them through as literal paths causes every later
    patch to fail with ``source_binding_missing_path``.  Keep valid paths
...
def _ingest_bare_offspring_artifact(data: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    """Wrap an exact task artifact when the runtime can bind it unambiguously.

    The model sometimes follows the task's frozen output schema instead of the
    Nexus envelope schema.  Preserve that object verbatim; only runtime-owned
    candidate and lineage fields are added.  Ambiguous plan binding or an
    unfrozen task remains a schema error at the caller.
    """

    binding = _single_offspring_plan_binding(payload)
    authority = _bare_artifact_authority(payload, data)
    if not data or binding is None or not authority:
        return data
    parent_ids, plan_id = binding
    ...
    return {
        "offspring": [
            {
                "artifact": dict(data),
                "artifact_type": _bare_artifact_type(payload),
                "concise_claim": "",
                "core_mechanism": "",
...
def _ingest_bare_seed_artifact(data: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    """Preserve a schema-authorized bare task artifact as a generation-0 seed."""

    authority = _bare_artifact_authority(payload, data)
    if not data or not authority:
        return data
    return {
        "candidates": [
            {
...
def _single_offspring_plan_binding(payload: dict[str, Any]) -> tuple[list[str], str] | None:
    parents = payload.get("parents")
    plans = payload.get("plans")
    if not isinstance(parents, list) or not isinstance(plans, list) or len(plans) != 1:
        return None
    selected_parent_ids = {
        str(parent.get("id") if isinstance(parent, dict) else getattr(parent, "id", "")).strip()
        for parent in parents
    }
    selected_parent_ids.discard("")
    plan = plans[0]
    plan_data = plan if isinstance(plan, dict) else plan.to_dict() if hasattr(plan, "to_dict") else {}
    parent_ids = list(dict.fromkeys(_as_string_list(plan_data.get("parent_ids"))))
    if not parent_ids or any(parent_id not in selected_parent_ids for parent_id in parent_ids):
        return None
    metadata = plan_data.get("metadata") if isinstance(plan_data.get("metadata"), dict) else {}
    plan_id = str(metadata.get("plan_id") or metadata.get("id") or "").strip()
    return parent_ids, plan_id

def _bare_artifact_authority(payload: dict[str, Any], artifact: dict[str, Any]) -> str:
    contract = payload.get("contract")
    contract_data = contract if isinstance(contract, dict) else contract.to_dict() if hasattr(contract, "to_dict") else {}
    dynamic_contract = contract_data.get("dynamic_artifact_contract")
    if not isinstance(dynamic_contract, dict):
        outcome_policy = contract_data.get("outcome_policy") if isinstance(contract_data.get("outcome_policy"), dict) else {}
        dynamic_contract = outcome_policy.get("dynamic_artifact_contract")
    required_sets: list[list[str]] = []
    if isinstance(dynamic_contract, dict):
        work_product = dynamic_contract.get("required_work_product")
        required = _as_string_list(work_product.get("required_fields")) if isinstance(work_product, dict) else []
        if required:
            required_sets.append(required)
        shapes = dynamic_contract.get("allowed_artifact_shapes")
        for shape in shapes if isinstance(shapes, list) else []:
            required = _as_string_list(shape.get("required_fields")) if isinstance(shape, dict) else []
            if required:
                required_sets.append(required)
        requirements = dynamic_contract.get("adapter_requirements")
        required = _as_string_list(requirements.get("required_fields")) if isinstance(requirements, dict) else []
        if required:
            required_sets.append(required)
    if required_sets:
        if any(all(field in artifact for field in required) for required in required_sets):
            return "contract.dynamic_artifact_contract"
        return ""

    frozen_spec = contract_data.get("frozen_spec") if isinstance(contract_data.get("frozen_spec"), dict) else {}
    problem_text = str(frozen_spec.get("problem_text") or "")
    spec_sha256 = str(frozen_spec.get("spec_sha256") or "")
    if problem_text and spec_sha256 == _text_sha256(problem_text) and _evaluator_led_artifact_ingestion(payload):
        return "external_evaluator+contract.frozen_spec"
    return ""

def _evaluator_led_artifact_ingestion(payload: dict[str, Any]) -> bool:
    plans = payload.get("plans")
    ...
    metadata = plan_data.get("metadata") if isinstance(plan_data.get("metadata"), dict) else {}
    return (
        metadata.get("plan_source") == "runtime_lineage_envelope"
        and metadata.get("completion_mode") == "complete_task_artifact_only"
    )

def _bare_artifact_type(payload: dict[str, Any]) -> str:
    contract = payload.get("contract")
    contract_data = contract if isinstance(contract, dict) else contract.to_dict() if hasattr(contract, "to_dict") else {}
    dynamic_contract = contract_data.get("dynamic_artifact_contract")
    if not isinstance(dynamic_contract, dict):
        ...
        dynamic_contract = outcome_policy.get("dynamic_artifact_contract")
    if isinstance(dynamic_contract, dict):
        artifact_type = str(dynamic_contract.get("artifact_type") or dynamic_contract.get("artifact_domain_label") or "").strip()
        if artifact_type:
            return artifact_type
    parents = payload.get("parents") if isinstance(payload.get("parents"), list) else []
    if parents:
        parent = parents[0]
        artifact_type = parent.get("artifact_type") if isinstance(parent, dict) else getattr(parent, "artifact_type", "")
...
def _repair_candidate_items(data: dict[str, Any], *, key: str) -> dict[str, Any]:
    ...
    for item in items:
        ...
        claimed_verification: dict[str, Any] = {}
        claimed_metadata = {
            field: metadata.pop(field)
            for field in ("evaluator", "verification_results")
            if field in metadata
        }
        for field in ("verification_result", "verification_trace", "preliminary_result"):
            if field in candidate:
                claimed_verification[field] = candidate.pop(field)
        if claimed_metadata:
            claimed_verification["metadata"] = claimed_metadata
        if claimed_verification:
            metadata["model_claimed_verification"] = claimed_verification
        if metadata:
...
__all__ = [
    "_ingest_bare_offspring_artifact", "_ingest_bare_seed_artifact", "_repair_array_response", "_repair_candidate_items", "_repair_objective_contract_response",
]

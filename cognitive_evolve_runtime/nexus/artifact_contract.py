...
class DynamicArtifactContract:
    ...
    def from_any(cls, value: Any, *, fallback_objective: str = "") -> "DynamicArtifactContract | None":
        ...
        required_fields = [str(item).strip() for item in _as_list(data.get("required_fields")) if str(item).strip()]
        field_constraints = _as_mapping(data.get("field_constraints"))
        required_work_product = _as_mapping(data.get("required_work_product") or data.get("work_product"))
        if not required_work_product and any((artifact_type, required_fields, field_constraints)):
            required_work_product = {
                "artifact_type": artifact_type,
                "required_fields": required_fields,
                "field_constraints": field_constraints,
            }
            for key in ("lineage_requirement", "materialization_rule"):
                if data.get(key) is not None:
                    required_work_product[key] = data[key]
        allowed_artifact_shapes = _as_mapping_list(data.get("allowed_artifact_shapes") or data.get("artifact_shapes"))
        if not allowed_artifact_shapes and any((artifact_type, required_fields, field_constraints)):
        ...
        if not final_gate and data.get("acceptance_rule") is not None:
            final_gate = {"acceptance_rule": data["acceptance_rule"]}
        return cls(
            objective=objective,
            artifact_domain_label=str(data.get("artifact_domain_label") or data.get("domain_label") or data.get("label") or artifact_type or "model_defined_artifact"),
            required_work_product=required_work_product,
            allowed_artifact_shapes=allowed_artifact_shapes,
            minimum_concrete_delta=_as_mapping(data.get("minimum_concrete_delta") or data.get("concrete_delta") or data.get("minimum_delta")),
            invalid_outputs=[str(item) for item in _as_list(data.get("invalid_outputs") or data.get("forbidden_substitutions"))],

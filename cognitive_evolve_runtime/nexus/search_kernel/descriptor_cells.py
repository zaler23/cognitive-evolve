...
def descriptor_cell(candidate: CandidateGenome) -> DescriptorCell:
    ...
    lineage_root = _bounded_identity(normalize_token(candidate.lineage[0] if candidate.lineage else candidate.id), 96) or "root"
    return DescriptorCell(
        artifact_family=artifact_family,
        mechanism_family=_bounded_identity(normalize_token(mechanism_family), 160) or "general",
...
def _family_descriptor_token(value: Any) -> str:
    ...
    if "#m" in text:
        return _bounded_identity(text, 320)
    return _bounded_identity(normalize_token(text), 160) or "general"
...
def _candidate_paths(candidate: CandidateGenome, *, patch_result: dict[str, Any], metadata: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for value in repair.get("target_files") or repair.get("paths") or []:
        ...
        if path:
            target.append(path)
    return {"source": list(dict.fromkeys(source)), "patch": list(dict.fromkeys(patch)), "target": list(dict.fromkeys(target))}


def _path_bucket(paths: list[str], *, prefix: str) -> str:
    if not paths:
        return f"{prefix}:none"
    unique = sorted(set(str(path) for path in paths if str(path or "").strip()))
    sample = "+".join(normalize_token(path)[-96:] or "path" for path in unique[:3])
    return f"{prefix}:{sample}#{stable_hash(unique)[:16]}"
...
def _verifier_modality(candidate: CandidateGenome) -> str:
    for item in reversed(candidate.verification_trace or []):
        ...
        if modality:
            return _bounded_identity(modality, 96)
    return "none"
...
def _failure_class(candidate: CandidateGenome) -> str:
    ...
    for key in ("failure_class", "hard_reject_class", "terminal_reject_reason", "hard_reject_reason"):
        ...
        if token:
            return _bounded_identity(token, 96)
...
def _bounded_identity(value: str, limit: int) -> str:
    ...
    if len(text) <= limit:
        return text
    return text[: max(1, limit - 17)] + "_" + stable_hash(text)[:16]

    ...
    def harvest(
        ...
        *,
        ...
        while (max_batches is None or batch_index < max_batches) and not result.stopped_reason:
            ...
            for current_index, batch, error in run_ordered_fanout(window, _request, max_workers=workers):
                ...
                if error is not None:
                    if on_error is not None and on_error(error):
                        # Drain already-started fanout results, then stop before
                        # opening another provider-call window.
                        result.fatal_model_error = result.fatal_model_error or error
            ...
            elif successful_batches_in_window <= 0 and first_recoverable_error is not None and not result.stopped_reason:
        ...
        for candidate in batch:
            ...
            if self.deduper.add(candidate):
                ...
                record = {
                    ...
                    "reason": reason,
                    "signature": candidate.metadata.get(signature_key),
                    "branch_slot_id": candidate.metadata.get("branch_slot_id", ""),
                    "branch_arm_id": candidate.metadata.get("branch_arm_id", ""),
                    "candidate": {
                ...
                record_candidate_budget_decision(candidate, source=f"{self.policy.stage}_harvester", reason=reason, action="soft_reservoir" if self.policy.reservoir_mode else "soft_reject", details=record)
    ...
    return stable_hash(
        {
            ...
            "instruction": str(plan.instruction or "").strip().lower(),
            "rarity_seed": str(plan.rarity_seed or "").strip().lower(),

...
def apply_seed_active_frontier(candidates: Iterable[CandidateGenome], *, limit: int = 64) -> dict[str, Any]:
    ...
    if len(items) <= max(1, int(limit or 1)):
        return {
            "schema": "seed_active_frontier.v2",
            "selected_ids": list(selected_ids),
            "advisory_unselected_count": 0,
            "policy": "pool_fits_frontier",
        }
    for candidate in items:
        ...
        if candidate.id in selected_ids:
            ...
            continue
        meta["seed_active_frontier"] = {
            "selected": False,
            "limit": int(limit or 1),
            "reason": "advisory_frontier_only_pending_observed_outcomes",
        }
    return {
        "schema": "seed_active_frontier.v2",
        "selected_ids": sorted(selected_ids),
        "advisory_unselected_count": len([candidate for candidate in items if candidate.id not in selected_ids]),
        "limit": int(limit or 1),
        "policy": "advisory_frontier_no_pre_evaluation_fate_change",
    }
...
def _open_metadata_inventory(prefix: str, metadata: dict[str, Any]) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
...
def _attachment_signals(candidate: CandidateGenome, *, factors: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    add("candidate.failure_lessons", 0.10 if candidate.failure_lessons else 0.0, candidate.failure_lessons)
    add("candidate.failure_theorem", 0.08 if any(item.get("source_candidate_id") == candidate.id for item in factors) else 0.0)
    add("candidate.formal_artifacts", 0.08 if candidate.formal_artifacts else 0.0, candidate.formal_artifacts)
    add("candidate.proof_obligations", 0.07 if candidate.proof_obligations else 0.0, candidate.proof_obligations)
    add("candidate.source_bindings", 0.06 if candidate.source_bindings else 0.0, candidate.source_bindings)
    add("candidate.evidence_refs", 0.06 if candidate.evidence_refs else 0.0, candidate.evidence_refs)
    add("candidate.edge_knowledge_seeds", 0.05 if candidate.edge_knowledge_seeds else 0.0, candidate.edge_knowledge_seeds)
    add("candidate.novelty_descriptors", 0.04 if candidate.novelty_descriptors else 0.0, candidate.novelty_descriptors)
...
def _family_key(candidate: CandidateGenome) -> str:
    ...
    for value in (
        ...
        if text:
            return text if len(text) <= 640 else text[:620] + "_" + stable_hash(text)[:16]
    return candidate.id

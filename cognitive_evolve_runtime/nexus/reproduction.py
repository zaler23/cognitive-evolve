...
def dedupe_offspring_against_population(
    ...
    *,
    ...
    for candidate in offspring:
        ...
        if deduper.add(candidate):
            ...
            candidate.metadata["duplicate_offspring_reason"] = reason
            if rejected_out is not None:
                rejected_out.append(
                    {
                        "candidate_id": candidate.id,
                        "reason": reason,
                        "branch_slot_id": str(candidate.metadata.get("branch_slot_id") or ""),
                        "branch_arm_id": str(candidate.metadata.get("branch_arm_id") or ""),
                    }
                )
            record_candidate_budget_decision(candidate, source="dedupe_offspring_against_population", reason=reason, action="hard_exclude")
    return unique
    ...
    *,
    ...
    for _, donor in donors[:limit]:
        ...
        child.metadata["elite_gap_merge"] = {
            ...
            "elite_gap_terms": sorted(elite_gaps),
            "donor_terms": sorted(_candidate_gap_terms(donor) | _candidate_evidence_terms(donor)),
...
def _mark_offspring_repair_incubating(candidate: CandidateGenome, payload: dict[str, Any], verdict: FailureVerdict | None = None) -> None:
    ...
    blockers = list(dict.fromkeys([*verdict.blockers, *(str(term) for term in terms)]))
    repair_targets = list(dict.fromkeys(verdict.repair_targets))
    ...
    for item in verdict.failure_guidance:
        ...
        guidance.append(fixed)
    if guidance:
        candidate.metadata["failure_micro_guidance"] = _dedupe_guidance(guidance)
    candidate.metadata["repair_required"] = {
        ...
        "source_bindings": [{"path": path, "kind": "source_file", "source": "offspring_failure_classifier"} for path in repair_targets],
        "next_actions": [

...
from cognitive_evolve_runtime.validation.result import canonical_validation_status
...
def bind_candidate_intent(candidate: CandidateGenome, *, contract: Any | None = None) -> dict[str, Any]:
    ...
    payload = {
        ...
        "supporting_claims": [str(item) for item in supporting if str(item or "").strip()],
        "alignment_rationale": str(rationale or _alignment_rationale(search_intent, main_claim, float(score or 0.0))),
    }
...
def candidate_verification_status(candidate: CandidateGenome) -> str:
    """Return a check-level status without claiming external correctness."""
    ...
    if result:
        return canonical_validation_status(result, default="inconclusive")
    ...
    if evidence_meta.get("final_blocked"):
        return "inconclusive"
    if result or evidence_meta:
        return "inconclusive"
    return "not_run"


def measured_verification_bonus(candidate: CandidateGenome, *, maximum: float = 0.08) -> float:
    """Bound selection influence to replayable, measured verifier evidence."""

    strength = candidate_verification_strength(candidate)
    return max(0.0, float(maximum)) * float(strength) / float(VerificationStrength.EXECUTABLE)
...
def user_facing_verification_status(
    ...
    *,
    ...
    """Return preliminary check status; external review remains out of process."""

    if _graded_preliminary_passed(graded_output, candidate_id=candidate.id):
        return "preliminary_passed"
    return candidate_verification_status(candidate)
...
def best_current_direction_payload(
    ...
    *,
    ...
    validation_status = user_facing_verification_status(candidate, final_certificate=final_certificate, graded_output=graded_output)
    return {
        ...
        "supporting_claims": list(binding.get("supporting_claims") or []),
        ...
        "external_review_required_reason": blocked_from_verified_claim_reason(candidate, user_status=validation_status),
    }
...
def best_current_direction_score(candidate: CandidateGenome, context: Iterable[CandidateGenome] | None = None, *, contract: Any | None = None) -> float:
    ...
    failed_penalty = 0.15 if status == "preliminary_failed" else 0.0
    return (
        0.38 * intent
        + 0.18 * quality
        ...
        - 0.10 * reskin
        - failed_penalty
        + measured_verification_bonus(candidate)
    )
...
def _canonical_mechanism_family_id(candidate: CandidateGenome) -> str:
    ...
    tokens = sorted(
        {
            token
            for token in (normalize_token(item) for item in _tokens(_text(candidate)))
...
def _main_claim(candidate: CandidateGenome) -> str:
    ...
    if isinstance(artifact, dict):
        for key in ("mechanism", "core_mechanism", "claim", "summary", "description", "content"):
            ...
            if text:
                return text
    return " ".join(str(artifact or "").split())
...
def _graded_preliminary_passed(graded_output: Any | None, *, candidate_id: str) -> bool:
    if graded_output is None:
        return False
    data = graded_output.to_dict() if hasattr(graded_output, "to_dict") else coerce_dict(graded_output)
    if str(data.get("mode") or "").lower() not in {"preliminary_result", "verified_result"}:
        return False
    result = coerce_dict(data.get("result"))
    replay = coerce_dict(data.get("replay_certificate"))
    return bool(result and replay and str(replay.get("candidate_id") or "") == str(candidate_id or ""))

"""Round-closure and stop-decision helpers for Nexus evolution."""
...
from cognitive_evolve_runtime.nexus.stop_reasons import is_solved_stop_reason
...
def _completion_status_for_budget(*, budget: EvolutionBudget, interrupted: bool, synthesis: SynthesizedResult) -> str:
    ...
    synthesis_status = str(getattr(synthesis, "status", "") or "").strip()
    normalized_synthesis_status = synthesis_status.lower()
    if reason in {"needs_continuation", "model_stop_unsolved_needs_continuation"}:
        return "needs_continuation"
    if normalized_synthesis_status == "best-current-route":
        return "best_current_route"
    if normalized_synthesis_status in {"best_current_route", "route_incomplete"}:
        return synthesis_status
    if "failure" in normalized_synthesis_status:
        return "failed"
    return "completed"
    ...
    *,
    ...
    checks = [
        ...
        },
        {
            ...
            "passed": terminal_status in {"completed", "solved", "best_current_route", "route_incomplete"},

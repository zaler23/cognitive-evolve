...
class ReproduceStage:
    ...
    def _select_reproduction_parents(
        ...
        *,
        ...
        admission_view = [*population.candidates, *(archive_admission_candidates or [])]
        parents = self.selector.select(admission_view, archives, limit=limit, eligibility_policy=_eligibility_policy(policy), advisory_features=advisory_features)
        if parents:

...
class AdaptiveRuntimeController:
    ...
    def attach_final_certificate(self, certificate: dict[str, Any]) -> None:
        if not self.enabled and not certificate:
            return
        self.state.final_certificate = {**certificate, "objective_solved": False}
        if certificate:
            self.state.record_event(adaptive_event("final_certificate", objective_solved=False, ready_for_external_review=bool(certificate.get("ready_for_external_review")), blocking_reasons=certificate.get("blocking_reasons", [])))

    def to_dict(self) -> dict[str, Any]:
        self.state.challenge_memory = self.challenge_memory.to_dict()
        data = self.state.to_dict()
        certificate = dict(data.get("final_certificate") or {})
        if certificate:
            data["final_certificate"] = {**certificate, "objective_solved": False}

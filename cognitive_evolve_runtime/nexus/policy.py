...
class EvolutionPolicy:
    ...
    search_space: dict[str, Any] = field(default_factory=dict, kw_only=True)
    ...
    def from_dict(cls, data: dict[str, Any]) -> "EvolutionPolicy":
        return cls(
            ...
            search_space=coerce_dict(data.get("search_space") or data.get("search_space_plan")),
        ...
        if model is not None and hasattr(model, "build_evolution_policy"):
            try:
                ...
                if isinstance(raw, EvolutionPolicy):
                    return _inherit_contract_search_space(raw, contract)
                if isinstance(raw, dict):
                    return _inherit_contract_search_space(EvolutionPolicy.from_dict(raw), contract)
...
def _inherit_contract_search_space(policy: EvolutionPolicy, contract: Any) -> EvolutionPolicy:
    """Keep a model policy from accidentally disabling the contract's search families."""

    typed_families = _search_space_family_mappings(policy.search_space)
    if typed_families:
        policy.search_space["candidate_families"] = typed_families
        return policy
    legacy_plan = coerce_dict(policy.metadata.get("search_space_plan"))
    legacy_families = _search_space_family_mappings(legacy_plan)
    if legacy_families:
        legacy_plan["candidate_families"] = legacy_families
        policy.metadata["search_space_plan"] = legacy_plan
        return policy
    contract_plan = _contract_search_space_plan(contract)
    policy.search_space = contract_plan or _objective_search_space_fallback(contract)
    return policy
...
def _fallback_policy_from_contract(
    ...
    *,
    ...
    policy = EvolutionPolicy(
        candidate_niches=list(dict.fromkeys(niches)),
        search_space=contract_plan or _objective_search_space_fallback(contract),
    )
...
def _contract_search_space_plan(contract: Any) -> dict[str, Any]:
    ...
    for source in (
        ...
        families = _search_space_family_mappings(plan)
        if families:
            inherited = dict(plan)
            inherited["candidate_families"] = families
            inherited.setdefault("source", "objective_contract")
            return inherited
    return {}


def _objective_search_space_fallback(contract: Any) -> dict[str, Any]:
    data = contract.to_dict() if hasattr(contract, "to_dict") else coerce_dict(contract)
    assessment = {
        **data,
        "real_objective": str(data.get("normalized_goal") or data.get("original_user_goal") or ""),
    }
    return build_search_space_map(assessment)


def _search_space_family_mappings(value: Any) -> list[dict[str, Any]]:
    plan = coerce_dict(value)
    raw = plan.get("candidate_families") or plan.get("exploration_planes") or plan.get("families") or plan.get("planes") or []
    if not isinstance(raw, list):
        return []
    families: list[dict[str, Any]] = []
    for item in raw:
        family = dict(item) if isinstance(item, dict) else {"id": str(item or "").strip()}
        if str(family.get("id") or family.get("name") or "").strip():
...
def _search_plane_ids_from_contract(contract: Any) -> list[str]:
    ...
    for source in sources:
        plan = coerce_dict(source)
        families = plan.get("candidate_families") or plan.get("exploration_planes") or plan.get("families") or plan.get("planes") or []

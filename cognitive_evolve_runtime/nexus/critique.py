...
Critiques are structured genome feedback that can survive checkpoints and steer
later reproduction without becoming a separate runtime authority.
"""
...
class CritiqueEngine:
        ...
        *,
        ...
        if self.model is not None and hasattr(self.model, "critique_candidates"):
            try:
                ...
                covered_ids = {critique.candidate_id for critique in critiques}
                for candidate in candidates:
                    if candidate.id in covered_ids:
                        continue
                    deterministic = self._deterministic_critique(candidate, round_index)
                    deterministic.metadata["coverage_source"] = "deterministic_model_omission"
                    critiques.append(deterministic)
                if critiques:
    ...
    def apply(self, *, candidates: list[CandidateGenome], critiques: list[CandidateCritique]) -> None:
        ...
        for critique in critiques:
            ...
            for gene in critique.reusable_genes:
                if gene and gene not in candidate.inherited_genes:
                    candidate.inherited_genes.append(gene)
            if critique.missing_evidence:
                candidate.missing_parts = list(dict.fromkeys(candidate.missing_parts + critique.missing_evidence))

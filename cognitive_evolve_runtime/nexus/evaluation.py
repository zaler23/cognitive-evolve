...
def _runtime_validation_report(task_dir: Path) -> dict[str, Any]:
    ...
    with snapshot_reader(nexus_dir) as snapshot_root:
        run_result = _load_optional_json(snapshot_root / "run-result.json")
        population = _load_optional_json(snapshot_root / "population.json")
        archives = _load_optional_json(snapshot_root / "archives.json")
        checkpoint = _load_optional_json(snapshot_root / "checkpoint.json")
        final_answer_exists = (snapshot_root / "final-answer.md").exists()
    events_path = nexus_dir / "events.jsonl"
    ...
    verification_results = state.get("verification_results", {}) if isinstance(state.get("verification_results"), dict) else {}
    ...
    checks = [
        ...
        _check(final_answer_exists, "final_answer_exists", "Nexus final answer artifact exists"),
        ...
        _check(
            verification_results.get("runtime_integrity_passed") is True,
            "nexus_runtime_integrity_passed",
            "Nexus runtime completed without integrity failures",
            {"runtime_integrity_passed": verification_results.get("runtime_integrity_passed"), "validation_status": validation_status},
        ),
    ...
    return build_suite_report(
        ...
        checks,
        generated_at=_now(),
        summary={"runtime_path": RUNTIME_PATH, "runtime_contract_version": RUNTIME_VERSION, "validation_status": validation_status},
    )
...
def native_optimize_run(path: str | None, *, source: str | None = None) -> int:
    ...
    if not task_dir.exists():
        ...
        return 1
    if source:
        source_text = _read(Path(source))
    else:
        with snapshot_reader(task_dir / "nexus-runtime") as snapshot_root:
            source_text = _read(snapshot_root / "final-answer.md")
        source_text = source_text or _read(task_dir / "intake" / "enhanced-task-contract.md")
...
def _offline_variants(source_text: str) -> list[dict[str, Any]]:
    base = " ".join(source_text.split())
    if not base:
        return []
    return [
        {"id": "nexus_optimize_focus", "strategy": "focus_objective", "prompt": f"Solve the task by first restating the objective, then producing the smallest verifiable answer. Context: {base}"},
        {"id": "nexus_optimize_evidence", "strategy": "evidence_first", "prompt": f"Separate input evidence, tool evidence, and model hypotheses before answering. Context: {base}"},
        {"id": "nexus_optimize_edge", "strategy": "rare_recall", "prompt": f"Generate mainstream, inversion, analogy, and rare-recall solution routes, then rank them relatively. Context: {base}"},
    ]

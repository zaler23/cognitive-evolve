...
class RepairSeed:
    ...
    def to_metadata(self, *, current_round: int) -> dict[str, Any]:
        targets = list(self.verdict.repair_targets)
        blockers = list(self.verdict.blockers)
        guidance = list(self.verdict.failure_guidance)
        return {
            "source": self.source,
            ...
            "blockers": blockers,
            "failure_lessons": list(self.candidate.failure_lessons),
...
def _candidate_from_failure_record(
    ...
    *,
    ...
    repair_required = {
        "blockers": blockers or [category],
        "evidence_needed": list(repair_seed["required_evidence"]),
        "source_bindings": [{"path": path, "kind": "source_file", "source": "failure_archive_reseed"} for path in targets],
        "next_actions": [
    ...
    return CandidateGenome(
        ...
        artifact={
            "source": "failure_archive_reseed",
            "original_candidate_id": original_id,
            "inherited_gene_summary": summary,
...
def _tokenize_family(value: str) -> str:
    text = re.sub(r"[^a-z0-9_./-]+", "_", str(value or "").lower()).strip("_") or "general"
    return f"{text[:80]}:{stable_hash({'canonical_family': text})}"

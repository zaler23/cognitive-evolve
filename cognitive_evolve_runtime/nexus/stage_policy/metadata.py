...
def _default_eligibility_policy() -> dict[str, Any]:
    # Import lazily so importing nexus.policy does not loop back through
    # candidates -> archives -> stage_policy -> nexus.policy.
...
def _max_incubation_attempts(metadata: dict[str, Any], policy_config: dict[str, Any] | None = None, diagnostics: set[str] | None = None) -> int:
    ...
    if parsed <= 0:
        parsed = _positive_int(_default_eligibility_policy().get("max_incubation_attempts"))
    if parsed > 0:
        return parsed
...
def _max_incubation_age(metadata: dict[str, Any], *, round_limit: int, policy_config: dict[str, Any] | None = None) -> int:
    ...
    if fraction is None or fraction <= 0:
        fraction = parse_metric_value(_default_eligibility_policy().get("max_incubation_age_fraction"))
    if fraction is None or fraction <= 0:
        fraction = 1.0
...
def _min_incubation_age_rounds(policy_config: dict[str, Any] | None = None) -> int:
    ...
    if parsed <= 0:
        parsed = _positive_int(_default_eligibility_policy().get("min_incubation_age_rounds"))
...
def _max_repeated_repair_blockers(metadata: dict[str, Any], policy_config: dict[str, Any] | None = None) -> int:
    ...
    if parsed <= 0:
        parsed = _positive_int(_default_eligibility_policy().get("max_repeated_repair_blockers"))
    return parsed if parsed > 0 else 1

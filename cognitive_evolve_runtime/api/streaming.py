...
from .engine_runner import _api_context_request, _api_request_record
...
def _stream_engine_chunks(prompt: str, *, request_id: str, model: str, raw_request: dict[str, Any]) -> Iterator[bytes]:
    ...
    def worker() -> None:
        try:
            ...
            _offer_event(events, {"type": "error", "error": f"CognitiveEvolve pipeline cancelled: {public_error_message(exc)}"}, block=True)
        except Exception as exc:
            _offer_event(events, {"type": "error", "error": f"CognitiveEvolve pipeline failed: {public_error_message(exc)}"}, block=True)
        finally:
    ...
    try:
        ...
        yield _stream_role_chunk(request_id=request_id, model=model, created=created)
        yield from _stream_content_chunks(request_id=request_id, model=model, created=created, content=f"CognitiveEvolve stream queue is full: {public_error_message(exc)}")
        yield _stream_done_chunk(request_id=request_id, model=model, created=created)
        yield b"data: [DONE]\n\n"
        return
    yield _stream_role_chunk(request_id=request_id, model=model, created=created)

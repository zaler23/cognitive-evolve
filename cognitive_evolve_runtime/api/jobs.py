...
from ..persistence.transactional_snapshot import snapshot_reader
...
def _job_public(job: dict[str, Any], *, include_answer: bool = True) -> dict[str, Any]:
    ...
    payload = {
        ...
        "resume_available": _job_checkpoint_available(job),
    }
    if include_answer and _job_has_result_payload(job):
        payload["answer"] = job.get("answer", "")
        from .payloads import (
            _nexus_actual_rounds,
            _nexus_answer_produced,
            _nexus_completion_status,
            _nexus_objective_solved,
            _nexus_ready_for_external_review,
            _nexus_validation_status,
            _nexus_verification_passed,
        )
        ...
        payload["cognitive_evolve"] = {
            ...
            "preliminary_validation_passed": _nexus_verification_passed(data),
            "validation_status": _nexus_validation_status(data),
            ...
            "ready_for_external_review": _nexus_ready_for_external_review(data),
...
def _job_checkpoint_available(job: dict[str, Any]) -> bool:
    root = _job_artifact_root(job)
    if root is None:
        return False
    try:
        with snapshot_reader(root / "nexus-runtime") as snapshot_root:
            return (snapshot_root / "checkpoint.json").exists()
...
def _set_job(job_id: str, **updates: Any) -> dict[str, Any]:
    with _JOBS_LOCK:
        ...
        snapshot = dict(job)
    root = _job_artifact_root(snapshot)
    if root is not None:
        try:
            _write_json(root / "job-status.json", _job_public(snapshot))
        except Exception:
...
def _rehydrate_job_from_artifact(job_id: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.warning("job_registry.rehydrate_status_failed id=%s error_type=%s", job_id, type(exc).__name__)
        return None
    ...
    try:
        with snapshot_reader(root / "nexus-runtime") as snapshot_root:
            nexus_path = snapshot_root / "run-result.json"
        ...
        logger.warning("job_registry.rehydrate_result_failed id=%s error_type=%s", job_id, type(exc).__name__)
    ...
    if request_path.exists():
        try:
            ...
            logger.warning("job_registry.rehydrate_request_failed id=%s error_type=%s", job_id, type(exc).__name__)
    return job
...
def _job_artifact_root(job: dict[str, Any]) -> Path | None:
    """Return the configured per-job directory, never a caller-selected path."""

    job_id = str(job.get("id") or "").strip()
    raw = job.get("artifact_root") or job.get("task_dir")
    if not job_id or Path(job_id).name != job_id or not raw:
        return None
    configured = get_service_config().api_task_root.expanduser().resolve()
    candidate = Path(str(raw)).expanduser().resolve()
    try:
        candidate.relative_to(configured)
    except ValueError:
        return None
    return candidate if candidate.parent == configured and candidate.name == job_id else None

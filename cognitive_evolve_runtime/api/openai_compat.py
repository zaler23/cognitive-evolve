...
The public contract is OpenAI-shaped for chat frontends and adds CognitiveEvolve
job/artifact endpoints:

- GET /health
- GET /v1/models
- POST /v1/cogev/jobs
- GET /v1/cogev/jobs/{job_id}
- POST /v1/cogev/jobs/{job_id}/resume
- DELETE /v1/cogev/jobs/{job_id}
- GET /v1/cogev/jobs/{job_id}/result
- GET /v1/cogev/jobs/{job_id}/artifacts
...
from .jobs import _cancel_job_future, _get_job, _job_has_result_payload, _job_public, _now, _pop_job_future, _register_job_future, _set_job, _status_from_nexus_data, _task_dir_for_request
...
def _install_job_routes(app: FastAPI) -> None:
    ...
    async def create_job(request: Request) -> JSONResponse:
        ...
        def worker() -> None:
            ...
            try:
                ...
                _set_job(job_id, status="cancelled", error=public_error_message(exc), cancellation_requested=True)
            except Exception as exc:
                _set_job(job_id, status="failed", error=f"CognitiveEvolve pipeline failed: {public_error_message(exc)}")

        try:
            future = get_job_executor().submit(worker)
        except QueueFullError as exc:
            rejected = _set_job(job_id, status="rejected", error=public_error_message(exc), cancellation_requested=False)
    ...
    async def resume_job(job_id: str, request: Request) -> JSONResponse:
        ...
        with snapshot_reader(output_dir) as snapshot_root:
            if not (snapshot_root / "checkpoint.json").exists():
        ...
        def worker() -> None:
            ...
            try:
                ...
                _set_job(job_id, status="failed", error=f"CognitiveEvolve resume failed: {public_error_message(exc)}")
    ...
    async def cancel_job(job_id: str) -> JSONResponse:
        ...
        if not job:
            raise HTTPException(status_code=404, detail="Unknown CognitiveEvolve job id.")
        if job.get("status") in {"completed", "best" + "_current" + "_route", "route" + "_incomplete", "failed", "cancelled", "needs_continuation", "failed_verification", "interrupted_checkpointed", "paused_quota"}:
            return JSONResponse(_job_public(job))
...
def _install_chat_routes(app: FastAPI) -> None:
    ...
    async def chat_completions(request: Request) -> Response:
        ...
        try:
            ...
            raise HTTPException(status_code=503, detail=public_error_message(exc)) from exc
        except Exception as exc:
            # Keep the error client friendly while not leaking upstream provider secrets.
            raise HTTPException(status_code=502, detail=f"CognitiveEvolve pipeline failed: {public_error_message(exc)}") from exc

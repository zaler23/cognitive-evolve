...
def _run_engine(
    ...
    *,
    ...
    _write_json(task_dir / "api-request.json", _api_request_record(request_id=request_id, model=model, raw_request=raw_request))
    ...
    with llm_session(LLMSession(journal_dir=str(llm_call_dir), call_ledger_path=str(llm_call_dir / "llm-call-ledger.jsonl"))), _temporary_model_runtime(model):
        result = EngineOrchestrator().run(
            ...
            context={
                ...
                "raw_request": _api_context_request(raw_request),
...
def _persist_raw_api_request() -> bool:
    return os.environ.get("COGEV_API_PERSIST_RAW_REQUEST", "").strip().lower() in {"1", "true", "yes", "on"}


def _api_context_request(raw_request: dict[str, Any]) -> dict[str, Any]:
    if _persist_raw_api_request():
        return redact(raw_request)
    return redact(
        {
            key: raw_request.get(key)
            for key in ("model", "metadata", "cognitive_evolve", "extra_body", "temperature", "max_tokens", "stream")
            if isinstance(raw_request, dict) and key in raw_request
        }
    )


def _api_request_record(*, request_id: str, model: str, raw_request: dict[str, Any]) -> dict[str, Any]:
    record = {
        "request_id": request_id,
        "model": model,
        "request_hash": stable_hash(redact(raw_request)),
        "raw_request_recorded": _persist_raw_api_request(),
    }
    if _persist_raw_api_request():
        record["request"] = redact(raw_request)
    else:
        record["request_summary"] = _api_context_request(raw_request)
    return record

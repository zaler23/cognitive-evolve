"""CLI-facing Nexus runtime commands.
...
def _artifact_policy_prompt_hint(adaptive_config: dict[str, object]) -> str:
    ...
    if allowed_terms:
        lines.append("- Prefer domain vocabulary: `" + "`, `".join(allowed_terms) + "`.")
    if forbidden_terms:
        lines.append("- Avoid forbidden semantic terms in the artifact: `" + "`, `".join(forbidden_terms) + "`.")

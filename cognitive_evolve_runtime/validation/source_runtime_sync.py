...
from .project_health import source_checkout_detected
...
def _iter_runtime_shared_source_files(runtime_root: Path) -> list[str]:
    ...
    for path in runtime_root.rglob("*"):
        ...
        if path.name in RUNTIME_COVERAGE_IGNORED_FILES:
            continue
        if rel in RUNTIME_ONLY_OPERATIONAL_FILES:
            continue
        parts = set(rel.split("/"))
...
def _source_runtime_coverage_validation() -> list[tuple[bool, str]]:
    ...
    if not source_checkout_detected(ROOT):
        return [(True, "source/runtime mirror coverage is not applicable to an installed runtime")]

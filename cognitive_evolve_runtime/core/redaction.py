...
_SECRET_WORD_RE = re.compile(r"(?i)\b(api[_-]?key|authorization|token|password|passwd|cookie|session[_-]?id|private[_-]?key)\b")
_LOCAL_PATH_RE = re.compile(
    r"(?i)(?:file://)?/(?:Users|home|root|private/var|var/folders|tmp)/[^\s\]\"'`),;<>]*"
)
...
def redact_text(text: str) -> str:
    ...
    redacted = _SECRET_WORD_RE.sub("[REDACTED_KEY]", redacted)
    redacted = _LOCAL_PATH_RE.sub("[REDACTED_PATH]", redacted)
    return redacted


def public_error_message(exc: BaseException | str, *, limit: int = 300) -> str:
    """Return a client-safe error string without endpoint, path, or credential detail."""
...
def _redact(value: Any, *, secrets: tuple[str, ...], parent_key: str, seen: set[int]) -> Any:
    ...
    if isinstance(value, str):
        ...
        text = _INLINE_SECRET_RE.sub(lambda m: (m.group(1) or "") + "[REDACTED]", text)
        return _LOCAL_PATH_RE.sub("[REDACTED_PATH]", text)
    return value
...
__all__ = ["public_error_message", "redact", "redact_text"]

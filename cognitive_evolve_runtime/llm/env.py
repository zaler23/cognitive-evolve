...
LLM_REASONING_EFFORT_ENV = "COGEV_LLM_REASONING_EFFORT"
DEFAULT_LLM_MAX_PROMPT_CHARS = 0
...
def llm_prompt_char_limit() -> int:
    """Return the provider-facing user-message cap."""

    return env_int(LLM_MAX_PROMPT_CHARS_ENV, DEFAULT_LLM_MAX_PROMPT_CHARS)


def effective_prompt_char_limit(requested_limit: int | None = None) -> int:
    """Combine a caller's structured-prompt cap with the transport cap.

    A non-positive transport cap keeps the legacy unlimited-transport behavior,
    while a positive cap remains authoritative and may only be narrowed by the
    caller.  This prevents a larger Nexus long-context setting from being
    silently truncated by transport later.
    """

    return int(prompt_char_limit_details(requested_limit)["effective"])


def prompt_char_limit_details(requested_limit: int | None = None) -> dict[str, int | bool | None]:
    """Expose which layer, if any, narrowed a requested prompt budget."""

    transport = max(0, llm_prompt_char_limit())
    requested = None if requested_limit is None else max(1, int(requested_limit))
    if requested is None:
        ...
        effective = min(requested, transport)
...
def normalize_reasoning_effort(value: Any, *, source: str = "reasoning_effort") -> str | None:
    """Return a canonical provider reasoning effort or fail closed."""
    ...
    if normalized not in SUPPORTED_REASONING_EFFORTS:
        ...
        raise LLMConfigurationError(f"{source} must be one of: {allowed}; got {normalized!r}.")
    return normalized
...
def llm_status() -> dict[str, Any]:
    ...
    reasoning_effort = normalize_reasoning_effort(os.environ.get(LLM_REASONING_EFFORT_ENV), source=LLM_REASONING_EFFORT_ENV)
    if provider == "fixture":
        return {
            ...
            "reasoning_effort_configured": reasoning_effort is not None,
        }
...
def llm_public_status(status: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for key, value in source.items():
        ...
        if key == "api_key_placeholder":
            ...
            continue
        if key == "api_base":
            public["api_base_configured"] = bool(str(value or "").strip())
            continue
        if key == "fixture":
            public["fixture"] = "configured" if value else ""
            continue
        public[key] = value
    return public

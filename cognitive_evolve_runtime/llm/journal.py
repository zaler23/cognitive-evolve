...
def write_llm_journal(record: dict[str, Any], *, raw_response: Any | None = None, parsed_response: dict[str, Any] | None = None) -> None:
    ...
    record = dict(record)
    with _JOURNAL_LOCK:
        if raw_response is not None and _write_raw_sidecars():
            ...
            record["raw_response_path"] = str(raw_path.relative_to(directory))
        if parsed_response is not None and _write_raw_sidecars():
            ...
            record["parsed_response_path"] = str(parsed_path.relative_to(directory))
        if raw_response is not None or parsed_response is not None:
            record["raw_sidecars_recorded"] = _write_raw_sidecars()
...
def _write_raw_sidecars() -> bool:
    return os.environ.get("COGEV_LLM_JOURNAL_RAW", "").strip().lower() in {"1", "true", "yes", "on"}

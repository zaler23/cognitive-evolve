...
class DirectHTTPProvider:
    ...
    def _completion(self, **kwargs: Any) -> Any:
        ...
        try:
            ...
            raise DirectHTTPProviderError(f"HTTP {exc.code} from direct_http provider", status_code=exc.code) from exc
        except urllib.error.URLError as exc:
            raise DirectHTTPProviderError(f"Network error from direct_http provider: {public_error_message(exc.reason)}") from exc
        except (TimeoutError, socket.timeout) as exc:
            raise DirectHTTPProviderError("Timeout from direct_http provider") from exc
        except Exception as exc:
            raise DirectHTTPProviderError(f"direct_http provider failed: {public_error_message(exc)}") from exc

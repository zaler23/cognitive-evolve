...
from .env import normalize_reasoning_effort
...
def identity_from_status(status: dict[str, Any], *, request_type: str) -> LLMCallIdentity:
    ...
    reasoning_effort = normalize_reasoning_effort(status.get("reasoning_effort"), source="llm_call_identity.reasoning_effort")
    return LLMCallIdentity(
        profile_id=profile_id,
        provider=provider,
        model=model,
        route_role=str(request_type or "default"),

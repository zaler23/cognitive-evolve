...
def completion_with_retry(completion_fn: Any, **kwargs: Any) -> tuple[Any, int]:
    ...
    allow_prompt_shrink = bool(kwargs.pop("_retry_allow_prompt_shrink", True))
    ...
    for attempts in range(1, max_attempts + 1):
        ...
        try:
            ...
            record = {
                ...
                "message": public_error_message(exc, limit=500),
            ...
            if attempts >= max_attempts or not retryable:
                ...
                _LAST_RETRY_HISTORY.set(retry_history)
                raise
            retry_mutation = _mutate_retry_kwargs(
                kwargs,
                category=last_category,
                allow_prompt_shrink=allow_prompt_shrink,
            )
...
def _mutate_retry_kwargs(
    kwargs: dict[str, Any],
    *,
    category: str,
    allow_prompt_shrink: bool = True,
) -> dict[str, Any]:
    ...
    cap = max(1, env_int(LLM_RETRY_MAX_TOKENS_ENV, 65536))
    current = _positive_int(kwargs.get("max_tokens"), default=16384)
    ...
    if allow_prompt_shrink and isinstance(messages, list):
        max_prompt_chars = max(1024, env_int(LLM_EMPTY_RETRY_PROMPT_CHARS_ENV, 500000))

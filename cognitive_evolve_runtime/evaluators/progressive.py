...
from cognitive_evolve_runtime.evaluators.artifact_normalizer import artifact_policy_from_config, normalize_artifact, semantic_drift_diagnostics, uses_artifact_policy
...
class ProgressiveEvaluator:
    def evaluate_result(self, candidate: Any, evaluator_result: EvaluatorResult | None, *, spec: EvaluatorSpec | None = None, round_index: int = 0) -> EvidenceRecord:
        ...
        normalizer_state = normalize_artifact(candidate, artifact_type=policy.artifact_type or getattr(candidate, "artifact_type", "") or "", policy=policy)
        ...
        provisional = EvidenceRecord(
            ...
            metadata={
                ...
                "artifact_normalizer_audit": {key: value for key, value in normalizer_state.items() if key not in {"artifact", "normalized_artifact"}} if external_artifact_authority else {},
...
def _external_evaluator_artifact_state(normalizer_state: dict[str, Any]) -> dict[str, Any]:
    return {
        **normalizer_state,
        "normalized_artifact": normalizer_state.get("artifact"),
        "status": "clean",
...
def _repair_hints(diagnostics: list[str], artifact_status: str) -> list[str]:
    ...
    for item in diagnostics:
        ...
        if text:
            hints.append(f"repair diagnostic: {text}")
    return hints

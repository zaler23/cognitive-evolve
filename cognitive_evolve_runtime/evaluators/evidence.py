...
def evidence_final_blocked(candidate: Any) -> bool:
    return bool(evidence_state(candidate).get("final_blocked", False))


def evidence_parent_blocked(candidate: Any) -> bool:
    return bool(evidence_state(candidate).get("parent_blocked", False))
...
def select_preliminary_incumbent(candidates: Iterable[Any]) -> Any | None:
    """Return the best candidate measured by the configured external evaluator.

    Progressive local probes intentionally do not qualify here.  The external
    evaluator boundary is recorded explicitly in ``metadata["evaluator"]`` by
    :class:`ExternalEvaluatorRunner`; once any such measurement exists, an
...
def _preliminary_incumbent_key(candidate: Any) -> tuple[int, float, int, int, int, str]:
    ...
    try:
        ...
        generation = 0
    try:
        created_round = int(metadata.get("created_in_round") or 0)
    except (TypeError, ValueError):
        created_round = 0
    return (
        tier,

...
class ChallengeMemoryItem:
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> "ChallengeMemoryItem | None":
        ...
        return cls(
            ...
            summary=str(data.get("summary") or data.get("kind") or case_id),
...
def challenge_from_diagnostic(*, candidate_id: str, source: str, diagnostic: str, round_index: int = 0, priority: float = 0.5) -> dict[str, Any]:
    summary = str(diagnostic or source or "challenge").strip() or "challenge"
...
def _challenge_items_from_record(record: EvidenceRecord) -> list[dict[str, Any]]:
    ...
    for diagnostic in record.diagnostics or record.hints:
...
def _mutation_instruction(
    ...
    *,
    ...
    summaries = "; ".join(item.summary for item in items if item.summary)
    ...
    if not schema_focus and semantic_focus:
        ...
        if allowed_terms:
            parts.append("Prefer domain terms from the allowed vocabulary: " + ", ".join(allowed_terms) + ".")

...
def aggregate_evidence_state(records: list[Any]) -> dict[str, Any]:
    ...
    latest_artifact_records = [
        ...
        if record is latest or (bool(latest_hash) and bool(evidence_artifact_hash(record)) and evidence_artifact_hash(record) == latest_hash)
    ]
    terminal_reject = any(bool(getattr(record, "terminal_reject", False)) for record in latest_artifact_records)
    ...
    for record in ordered:
        ...
        same_artifact = record is latest or (bool(latest_hash) and bool(record_hash) and record_hash == latest_hash)
    ...
    top_authority = max(latest_artifact_records, key=evidence_authority_rank)
    return {

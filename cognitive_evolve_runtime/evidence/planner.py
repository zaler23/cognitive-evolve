...
from ..core.redaction import public_error_message, redact, redact_text
...
def _safe_evidence_payload(source_type: str, *, assessment: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    prompt = assessment.get("real_objective") or assessment.get("surface_request")
    payload = {
        ...
        "context": {
            ...
            if key in {"api_request_id", "interface"}
        },
    }
    if os.environ.get("COGEV_EVIDENCE_SEND_PROMPT", "").strip().lower() in {"1", "true", "yes", "on"}:
        payload["query"] = redact_text(str(prompt or ""))
        payload["surface_request"] = redact_text(str(assessment.get("surface_request") or ""))
    return payload
...
class MCPStdioEvidenceAdapter:
    ...
    def execute(self, source_type: str, *, assessment: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"MCP evidence adapter failed: {public_error_message(exc)}") from exc
        if completed.returncode != 0:
            stderr = redact_text(completed.stderr.decode("utf-8", errors="replace"))[:500]
...
class HTTPJsonEvidenceAdapter:
    ...
    def execute(self, source_type: str, *, assessment: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"HTTP evidence adapter failed: {public_error_message(exc)}") from exc
    ...
    def _resolve_endpoint_addresses(self, host: str, port: int) -> set[str]:
        ...
        try:
            ...
            raise ValueError("HTTP evidence endpoint host could not be resolved safely.") from exc
    ...
    def _execute_adapter_or_gap(self, source_type: str, assessment: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "summary": public_error_message(exc),
                "claims_supported": [],
                "claims_unverified": [assessment.get("real_objective", "external fact dependency")],
            }
...
class EvidenceStore:
    ...
    def write_plan(
        ...
        sources = {
            ...
            "workspace_root": redact_text(str(execution.get("workspace_root") or "")) or None,
            "results": redact(execution.get("results", [])),
        }
        ledger = {
            ...
            "execution_status": execution.get("status", "not_executed"),
            "execution_results": redact(execution.get("results", [])),
        }
        ...
        with tool_calls.open("w", encoding="utf-8") as handle:
            for record in records:
                handle.write(json.dumps(redact({"time": _now(), **record}), ensure_ascii=False) + "\n")
        return {

...
class RelativeRankingResult:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "RelativeRankingResult":
        ...
        if observation_warnings:
            raw_notes = (raw_notes + "; " if raw_notes else "") + "ranking_schema_repair:" + ",".join(observation_warnings)
        return cls(
...
def _rank_order_audit_enabled(candidates: list[CandidateGenome]) -> bool:
    mode = os.environ.get("COGEV_RANK_ORDER_AUDIT", "off").strip().lower()
    if mode in {"off", "0", "false", "no"} or len(candidates) <= 1:
        return False
    if mode in {"always", "1", "true", "yes"}:
        return True
    # Explicit deterministic sample; production and unknown modes stay single-pass.
    return mode == "sample" and len(candidates) <= 4
...
def _verification_score(candidate: CandidateGenome) -> float | None:
    ...
    if isinstance(result, dict):
        ...
        passed = preliminary_passed_from_mapping(result)
        if passed is True:
            return 1.0
        if passed is False:
            return 0.25

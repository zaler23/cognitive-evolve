...
class VerificationResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if validation_status not in {"not_run", "preliminary_passed", "preliminary_failed", "inconclusive"}:
            validation_status = "preliminary_passed" if self.passed else "preliminary_failed"
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> "VerificationResult":
        ...
        metadata["validation_status"] = canonical_validation_status({**metadata, **raw}, default="inconclusive")
        for key in (
            "measured_strength",
            "measured_strength_value",
            "honesty_measurements",
...
class GradedOutput:
    ...
    def __post_init__(self) -> None:
        ...
        # ``verified_result`` is a read-compatible legacy spelling. Producers
        # can only serialize a preliminary local result or a portfolio.
        if self.mode == "verified_result":
            self.mode = "preliminary_result"
        if self.mode not in {"preliminary_result", "graded_portfolio"}:
            raise AssertionError("graded output mode must be preliminary_result or graded_portfolio")
        if self.mode == "preliminary_result":
            assert self.result is not None
        if self.result is not None:
            assert self.mode == "preliminary_result"
            assert self.verification_strength >= VerificationStrength.FORMAL
            assert self.result.replayable and self.replay_certificate

    def to_dict(self) -> dict[str, Any]:
        validation_status = "preliminary_passed" if self.result is not None else (
            "not_run" if self.verification_strength == VerificationStrength.NONE else "inconclusive"
        )
        return {
            "mode": self.mode,
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> "GradedOutput":
        ...
        mode = str(raw.get("mode") or ("preliminary_result" if result is not None else "graded_portfolio"))
        if mode == "verified_result":
            mode = "preliminary_result"
        if mode == "preliminary_result" and result is None:

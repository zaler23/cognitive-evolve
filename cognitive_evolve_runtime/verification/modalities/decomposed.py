...
class DecomposedVerifier:
    ...
    def check(self, candidate: Any) -> VerificationResult:
        ...
        if not self.required_claims:
            return VerificationResult(False, score=0.0, strength=self.strength, replayable=False, diagnostics=["no_required_claims"], metadata={"fingerprint": self.fingerprint})
        ...
        return VerificationResult(score >= 1.0, score=score, strength=self.strength, evidence_ref="evidence-" + stable_hash({"claims": self.required_claims, "passed": passed_claims})[:16], replayable=True, diagnostics=[f"claims_passed:{len(passed_claims)}/{len(self.required_claims)}"], metadata={"fingerprint": self.fingerprint})

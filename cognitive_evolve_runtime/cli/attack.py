...
def run_attack(*, problem_path: str | None, budget: int | None, out_dir: str | None, resume_dir: str | None = None, offline: bool = False) -> int:
    ...
    try:
        if resume_dir:
            ...
            result = runtime.run_text(prompt, user_goal=str(problem.get("goal") or problem.get("objective") or prompt), max_rounds=max(1, int(budget or problem.get("budget") or 1)), adaptive_config=adaptive)
    except Exception as exc:

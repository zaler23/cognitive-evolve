...
from cognitive_evolve_runtime.inputs.project_snapshot import GENERATED_SUFFIXES, IGNORE_DIRS, SENSITIVE_FILE_NAMES
...
_SANDBOX_SENSITIVE_NAMES = SENSITIVE_FILE_NAMES | {
    ".env.development",
    ".env.local",
    ".env.production",
    ".env.test",
...
class PatchSandbox:
    ...
    def prepare(self, candidate_id: str) -> Path:
        candidate_id, target = self._candidate_target(candidate_id)
        if self.sandbox_root == self.source_root:
            raise ValueError("sandbox_root must not equal source_root")
        lock_path = self.sandbox_root / f".{candidate_id}.sandbox.lock"
        with file_lock(lock_path):
            if target.exists():
                shutil.rmtree(target)
            ignore = lambda directory, names: _sandbox_copy_ignore(
                directory,
                names,
    ...
    def apply(self, candidate: CandidateGenome) -> PatchApplicationResult:
        candidate_id, _target = self._candidate_target(candidate.id)
        lock_path = self.sandbox_root / f".{candidate_id}.apply.lock"
        with file_lock(lock_path):
            return self._apply_locked(candidate)

    def _candidate_target(self, candidate_id: str) -> tuple[str, Path]:
        if not isinstance(candidate_id, str) or not candidate_id.strip():
            raise ValueError(f"unsafe candidate id: {candidate_id!r}")
        if candidate_id in {".", ".."} or "/" in candidate_id or "\\" in candidate_id or Path(candidate_id).is_absolute():
            raise ValueError(f"unsafe candidate id: {candidate_id!r}")
        ...
        try:
            relative = resolved_target.relative_to(self.sandbox_root)
        except ValueError as exc:
            raise ValueError(f"unsafe candidate id: {candidate_id!r}") from exc
        if not relative.parts:
            raise ValueError(f"unsafe candidate id: {candidate_id!r}")
        try:
            self.source_root.relative_to(resolved_target)
        except ValueError:
            pass
        else:
            raise ValueError(f"unsafe candidate id: {candidate_id!r}")
        ...
        if not patch_set:
            ...
            if generic_patch:
                ...
                if ok:
                    ...
                    diagnostics.append(message)
        post_hash = _sandbox_tree_hash(sandbox)
        if failed:
            status = "failed"
        elif not applied:
            status = "no_op"
...
def _sandbox_copy_ignore(directory: str, names: list[str], *, source_root: Path, sandbox_root: Path) -> set[str]:
    ignored: set[str] = set()
    root = Path(directory)
    for name in names:
        path = root / name
        if _sandbox_copy_ignored(path, source_root) or _path_within(path, sandbox_root):
            ignored.add(name)
    return ignored
...
def _sandbox_copy_ignored(path: Path, source_root: Path) -> bool:
    try:
        relative = path.relative_to(source_root)
        path.resolve().relative_to(source_root.resolve())
    except (OSError, ValueError):
        return True
    if path.is_symlink() or set(relative.parts) & IGNORE_DIRS:
        return True
    if path.name in _SANDBOX_SENSITIVE_NAMES:
        return True
    if any(
        fnmatch.fnmatch(path.name, pattern) or fnmatch.fnmatch(relative.as_posix(), pattern)
        for pattern in _SANDBOX_SENSITIVE_PATTERNS
    ):
        return True
    return path.suffix.lower() in GENERATED_SUFFIXES


def _sandbox_tree_hash(root: Path) -> str:
    files: dict[str, str] = {}
...
def preflight_unified_patch(patch_text: str) -> dict[str, Any]:
    ...
    for line in lines:
        if line.startswith("@@") and not _HUNK_RE.match(line):
            diagnostics.append("malformed_hunk_header:" + line)
    ...
    for raw in raw_paths:
        ...
        if path and (Path(path).is_absolute() or ".." in Path(path).parts):
            diagnostics.append("unsafe_patch_path:" + path)

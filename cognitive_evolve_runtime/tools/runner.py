...
class ToolRunner:
    def __init__(
        ...
        *,
        ...
        allowed_executable_paths: set[str | Path] | None = None,
    ) -> None:
    ...
    def run(self, command: str | list[str], *, cwd: str | Path, env: dict[str, str] | None = None, timeout_seconds: float | None = None) -> ToolFeedback:
        ...
        try:
            ...
            return ToolFeedback(
                ...
                raw_output_ref=redact_text(proc.stdout + proc.stderr),
            )
            ...
            return ToolFeedback(
                ...
                cost={"seconds": round(time.monotonic() - start, 4)},
                confidence=0.5,
                raw_output_ref=redact_text(str(exc)),
            )
    ...
    def _command_allowed(self, args: list[str]) -> tuple[bool, str]:
        ...
        if executable.expanduser().resolve() == Path(sys.executable).resolve():
            return True, ""
        if executable.parent != Path("."):
            if name in self.allowed_executables and executable.expanduser().resolve() in self.allowed_executable_paths:
                return True, ""
            return False, f"verifier command path not allowlisted: {args[0]}"
        if name in self.allowed_executables:
            return True, ""

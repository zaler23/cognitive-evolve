...
class ArchiveManager:
    ...
    def assign_by_policy(
        ...
        *,
        ...
        for candidate in candidates:
            ...
            evidence_terminal = evidence_terminal_reject(candidate)
            ...
            task_evaluator_passed = evaluator.get("passed") is True or evaluator_status in {"passed", "pass", "ok", "success"}
            evidence_blocks_search = evidence_blocks_parent or evidence_terminal or (evidence_blocks_final and not task_evaluator_passed)
            if current_fate in TERMINAL_FAILURE_FATES and structurally_blocked(candidate):
                fate = current_fate
            elif evidence_terminal and not evidence_repairable:
            ...
            elif _candidate_verification_blocks_final(candidate) or evidence_blocks_search:
            ...
            if _candidate_verification_blocks_final(candidate) or evidence_blocks_search:
    ...
    def is_final_answer_eligible(self, candidate: CandidateGenome) -> bool:
        ...
        if evidence_final_blocked(candidate):
            return False
        ...
        if (
            ...
            records.append(
                ArchiveConstraintRecord(
                    ...
                    severity="warning",
                    evidence={"fate": assignment.fate, "failure_lessons": list(candidate.failure_lessons)},
                )
            )
        for lesson in candidate.failure_lessons:
            records.append(
                ArchiveConstraintRecord(
...
def _evidence_failure_signature(candidate: CandidateGenome) -> str:
    ...
    return "evidence:" + stable_hash(
        {
            "stage": evidence.stage,
            "source": evidence.source,
            "emitted_challenge_ids": evidence.emitted_challenge_ids,
        }
    )

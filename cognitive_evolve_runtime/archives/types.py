...
class TerminalCandidateTombstone:
    ...
    def from_candidate(cls, candidate: CandidateGenome, *, fate: str, failure_signature: str = "") -> "TerminalCandidateTombstone":
        ...
        verification_payload = {
            ...
            "verification_trace": candidate.verification_trace,
            "failure_lessons": candidate.failure_lessons,
        }
        return cls(
            ...
            failure_signature=str(
                failure_signature
                or _verification_failure_signature(candidate)
                or ("failure_lessons:" + stable_hash({"failure_lessons": candidate.failure_lessons}) if candidate.failure_lessons else "")
            ),
        )

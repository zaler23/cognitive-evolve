...
class NexusCheckpoint:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "NexusCheckpoint":
        data = upgrade_checkpoint_payload(data)
        return cls(
...
def upgrade_checkpoint_payload(data: dict[str, Any]) -> dict[str, Any]:
    """Upgrade the only supported legacy checkpoint shape to the current one."""
    ...
    if version > CHECKPOINT_SCHEMA_VERSION:
        raise ValueError(
            f"checkpoint schema_version {version} is newer than supported version {CHECKPOINT_SCHEMA_VERSION}"
        )
    if version < 0:
        raise ValueError("checkpoint schema_version must not be negative")
    if version == 0:
...
__all__ = [
    ...
    "contract_payload_for_persistence",

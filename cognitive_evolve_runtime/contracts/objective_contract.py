...
def _frozen_spec_from_text(problem_text: str) -> dict[str, str]:
...
def _canonical_frozen_spec(value: Any) -> dict[str, Any]:
    data = coerce_dict(value)
    if "problem_text" not in data:
        return {}
    return _frozen_spec_from_text(str(data.get("problem_text") or ""))


def _bind_frozen_text_spec(contract: "NexusObjectiveContract", *, packet: Any, fallback_text: str) -> None:
    problem_text = str(getattr(packet, "raw_text", "") or fallback_text or "")
    contract.original_user_goal = problem_text
...
class NexusObjectiveContract:
    ...
    search_space_plan: dict[str, Any] = field(default_factory=dict, kw_only=True)
    ...
    def __post_init__(self) -> None:
        self.frozen_spec = _canonical_frozen_spec(self.frozen_spec)
        self.search_space_plan = coerce_dict(self.search_space_plan)
    ...
    def from_dict(cls, data: dict[str, Any]) -> "NexusObjectiveContract":
        return cls(
            ...
            frozen_spec=_canonical_frozen_spec(data.get("frozen_spec")),
            search_space_plan=coerce_dict(data.get("search_space_plan") or data.get("search_space")),
    ...
    def canonical_payload(self) -> dict[str, Any]:
        ...
        if not data.get("frozen_spec"):
            data.pop("frozen_spec", None)
        return data
...
class NexusObjectiveContractBuilder:
    ...
    def build_text_contract(
        ...
        *,
        ...
        frozen_spec = _frozen_spec_from_text(str(getattr(packet, "raw_text", "") or user_goal))
        contract = NexusObjectiveContract(
            original_user_goal=frozen_spec["problem_text"],
...
def _coerce_dynamic_artifact_contract(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("dynamic_artifact_contract", "artifact_contract", "model_artifact_contract"):
        ...
        if isinstance(value, dict) and value:
            ...
            return {**value, **dac.to_dict()} if dac is not None else dict(value)

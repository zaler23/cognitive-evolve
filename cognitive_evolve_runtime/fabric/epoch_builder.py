...
def build_round_parity_epoch_graph(*, round_index: int, include_preprocess: bool = False) -> TaskGraph:
    suffix = f"r{int(round_index):04d}"
    graph = TaskGraph(epoch=int(round_index or 0))
    evaluate_deps: list[str] = []
    if include_preprocess:
        preprocess_id = f"preprocess-{suffix}"
        graph.add(ExplorationTask(task_id=preprocess_id, kind=TaskKind.PREPROCESS, payload={"planned_round": int(round_index or 0)}, epoch=int(round_index or 0), model_pool="default"))
        evaluate_deps.append(preprocess_id)
    graph.add(ExplorationTask(task_id=f"evaluate-{suffix}", kind=TaskKind.EVALUATE, depends_on=evaluate_deps, payload={"planned_round": int(round_index or 0)}, epoch=int(round_index or 0)))

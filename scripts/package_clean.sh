...
rsync -a ./ "$tmp/$name/" \
  ...
  --exclude '*.egg-info/' \
  --exclude 'runtime/' \
  --exclude 'nexus-runtime/' \
  --exclude 'logs/' \
  --exclude 'adaptive/' \
  --exclude 'research/' \
  --exclude 'patch-sandboxes/' \
  --exclude 'evaluations/' \
  --exclude '.cogev/tasks/' \
  --exclude '.cogev/api-runs/' \
  --exclude 'checkpoint.json' \
  --exclude 'runtime-state.json' \
  --exclude 'population.json' \
  --exclude 'events.jsonl' \
  --exclude 'llm-call-ledger*.jsonl' \
  --exclude '*.sqlite' \
  --exclude '*.db' \
  --exclude '__pycache__/' \
  --exclude '*.pyc' \
...
for pattern in '._*' '__MACOSX' '.git' '.env' '.env.local' '.pytest_cache' '__pycache__' '.coverage' \
  'nexus-runtime' 'logs' 'adaptive' 'research' 'patch-sandboxes' 'evaluations' \
  'checkpoint.json' 'runtime-state.json' 'population.json' 'events.jsonl' 'llm-call-ledger*.jsonl' '*.sqlite' '*.db'; do

...
1. Read `README.md`, `AGENTS.md`, `LOCAL_RUNTIME_BOUNDARY.md`, and the relevant files under `cognitive_evolve_runtime/resources/specs/`.
...
- A source/runtime boundary note if files under `scripts/`, `cognitive_evolve_runtime/`, or `cognitive_evolve_runtime/tools/` change.

Search, ranking, prompt, or evaluator changes must also state the held-out
evaluation protocol. Do not train, tune, or select a mechanism on its reported
test set. Capability claims require sealed tasks, equal model and token budgets
for the baseline, and independent scoring; local fixtures and one-shot answers
only establish implementation behavior. A new search mechanism should replace
the weaker heuristic it supersedes rather than accumulate as another control
path, and benchmark-specific branches are not accepted.

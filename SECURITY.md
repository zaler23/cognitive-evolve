...
- Model-generated artifacts are not executed automatically. Synthesized
  verifier plans cannot enable host execution.
- A directly configured operator-owned command is explicit opt-in, remains a
  preliminary check, and is **not** an operating-system sandbox. The operator
  is responsible for running untrusted code in a mature isolated environment.
- Project verification defaults to non-executing syntax/static checks. The
  explicit `--include-tests` / `COGEV_VERIFY_INCLUDE_TESTS` operator opt-in may
  execute repository-defined pytest or npm test code and therefore requires an
  externally isolated environment for untrusted projects.
- Local operator commands remain allowlisted. Add new commands through

...
| `COGEV_LLM_REQUIRED_MODEL` | Optional comma-separated allowlist that rejects a resolved real-model route before the call when it does not match the operator-selected model |
| `COGEV_LLM_REASONING_EFFORT` | Optional validated API reasoning effort (`none`, `minimal`, `low`, `medium`, `high`, `xhigh`, or `max`); included in call identity and idempotency; the selected model remains authoritative |
...
Resume continues the stored population and must not rerun the original seed
phase; start a fresh run when a new generation-zero portfolio is intended.
Stored runtime options remain authoritative on resume. Checkpoints created before
the options snapshot use `single_batch` and `sync_full` compatibility semantics.

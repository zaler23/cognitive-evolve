...
Nexus produces candidate artifacts, lineage, and explicitly preliminary local
signals. It never converts its own evaluator, verification, certificate, or
review-bundle metadata into final correctness: producer-owned artifacts keep
`objective_solved=false`, while an independent reviewer owns acceptance.
...
- Declaring external frameworks as runtime backends instead of adapter/tool
  integrations.
- Adding a second finality authority, benchmark service, or validation control
  plane inside the candidate-producing runtime.

## Current artifact boundary

Nexus loop artifacts are written beneath `nexus-runtime/`. Task-root artifacts
also include `runtime-state.json` from `nexus/state.py` and `evaluations/`
reports such as `native-eval-report.json`. Final snapshot files are published
with a `snapshot-transaction.json` manifest; append-only event logs remain
outside that snapshot and are de-duplicated by logical event identity.

...
| TD-V3-LOOP-HIGH-CAPABILITY-ROUTING | Closed in protocol + tests | Public LOOP protocol requires all real self-bootstrap roles to resolve to one high-capability profile before provider calls; route values stay run-local rather than embedded as source constants. |
